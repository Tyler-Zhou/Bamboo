...
{   
    ...
    /// </summary>
    [ToolboxItem(false)]
    public partial class TrialBalanceSearchPart : ReportBaseSearchPart
    {
        #region Service
        ...
        private string _DetailForTrialBalanceName = "DetailForTrialBalance";
        ...
        public override object GetData()
        {
            ...
            try
            {
                _SearchParameter = new SearchParameter { To = DateTime.Parse(dteTo.DateTime.ToShortDateString()), CompanyIDs = companyIDs, BillTypes = billTypes };
                List<GLData> list = FinanceReportService.GetTrialBalance(
                  Utility.GetEndDate(_SearchParameter.To)
                  , _SearchParameter.CompanyIDs.ToArray()
                  , _SearchParameter.BillTypes.ToArray());

                List<ReportParameter> paramList = new List<ReportParameter>();
                paramList.Add(new ReportParameter("EndDate", dteTo.DateTime.ToShortDateString()));
                ...
                rd.ReportName = "ICP.ReportCenter.UI.Comm.LocalReports.TrialBalanceReport.rdlc";
                rd.Parameters = paramList;
                List<ReportDataSource> ds = new List<ReportDataSource>();
                ds.Add(new ReportDataSource("TrialBalanceData", list));
                rd.DataSource = ds;
                return rd;
            }
            catch (Exception ex)
        ...
        public override ReportData GetDrillthroughData(string reportEmbeddedResource, IList<ReportParameter> parameters)
        {
            ...
            if (reportEmbeddedResource.Contains(_DetailForTrialBalanceName))
            {
        ...
        ReportData GetDatailData(IList<ReportParameter> parameters)
        {
            try
            {
                ...
                ReportData rd = new ReportData();
                List<ReportDataSource> ds = new List<ReportDataSource>();
                ds.Add(new ReportDataSource("LedgerDataList", data));
                rd.DataSource = ds;
                return rd;
            }
            catch (Exception ex)
