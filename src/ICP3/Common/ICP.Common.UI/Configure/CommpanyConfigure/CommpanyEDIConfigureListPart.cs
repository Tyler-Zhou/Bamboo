...
namespace ICP.Common.UI.Configure.CommpanyConfigure
{
    ...
    public partial class CommpanyEDIConfigureListPart : BaseListEditPart
    {
        #region 初始化

        public CommpanyEDIConfigureListPart()
        {
            InitializeComponent();
            this.Enabled = false;
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            commStyleFormatCondition.Appearance.Font = new Font(colServiceConfigureKeyName.AppearanceCell.Font, FontStyle.Bold);
        ...
        private void txtFind_TextChanged(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                expressionBulider.Append("Upper([ServiceConfigureKeyName]) Like '%");
                ...
                List<CompanyEDIConfigureListClient> list = bsList.DataSource as List<CompanyEDIConfigureListClient>;
                List<CompanyEDIConfigureListClient> tagers = list.FindAll(delegate(CompanyEDIConfigureListClient item)
                {
                    return item.ServiceConfigureKeyName.ToUpper().Contains(txtFind.Text.Trim().ToUpper());
                });
                ...
                {
                    CompanyEDIConfigureListClient checkData = gvMain.GetRow(i) as CompanyEDIConfigureListClient;
        ...
        private void barSave_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            DialogResult dlg = DevExpress.XtraEditors.XtraMessageBox.Show(LocalData.IsEnglish ? "Are you sure combine?" : "确认要选择这些EDI配置?", LocalData.IsEnglish ? "Confirm" : "确认", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
            ...
            try
            {
                List<Guid> ediConfigureIDs = new List<Guid>();
                foreach (CompanyEDIConfigureListClient data in bsList.List)
                {
                    if (data.IsSelected)
                    {
                        ediConfigureIDs.Add(data.ID);
                    }
                }

                this.Controller.SaveCompanyEDIConfigureInfo(_currentConfigureList.ID
                    , ediConfigureIDs.ToArray()
        ...
        void SetListViewStyle(ViewStyle viewStyle)
        {
            ...
            else if (viewStyle == ViewStyle.Selected)
            {
                ...
                List<CompanyEDIConfigureListClient> selectedList = _originalList.FindAll(delegate(CompanyEDIConfigureListClient item)
                {
            ...
            else
            {
                ...
                List<CompanyEDIConfigureListClient> unSelectedList = _originalList.FindAll(delegate(CompanyEDIConfigureListClient item)
                {
        ...
        private void gvMain_CellValueChanged(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs e)
        {
            ...
            CompanyEDIConfigureListClient checkData = gvMain.GetRow(e.RowHandle) as CompanyEDIConfigureListClient;
        ...
        List<CompanyEDIConfigureListClient> _originalList = new List<CompanyEDIConfigureListClient>();
        ...
        public override void Init(IDictionary<string, object> values)
        {
            if (values != null && values.ContainsKey("ConfigureList"))
            {
                ...
                {
                    List<EDIConfigureList> edilistTotal = this.Controller.GetEDIConfigureList(true);
                    foreach (EDIConfigureList r in edilistTotal)
                    {
                        CompanyEDIConfigureListClient tmp = new CompanyEDIConfigureListClient();
                ...
                {
                    List<CompanyEDIConfigureList> edilistByCompany = this.Controller.GetCompanyEDIConfigureList(_currentConfigureList.ID, true);
                    if (edilistByCompany != null)
                    {
                        for (int i = 0; i < edilistByCompany.Count; i++)
                        {
                            CompanyEDIConfigureListClient item = _originalList.Find(delegate(CompanyEDIConfigureListClient t) { return t.ID == edilistByCompany[i].ID; });
                            if (item != null)
                            {
    ...
    public class CompanyEDIConfigureListClient : CompanyEDIConfigureList
    {
