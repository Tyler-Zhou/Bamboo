...
namespace ICP.FCM.OceanImport.UI
{
    public class Utility
    {
        ...
        public static IFileConvertService FileConvertService
        {
            get
            {
        ...
        static public void EnsureDefaultCompanyExists(ICP.Sys.ServiceInterface.IUserService userService)
        {
            if (string.IsNullOrEmpty(LocalData.UserInfo.DefaultCompanyName))
            {
                ...
                {
                    throw new Exception(LocalData.IsEnglish ? "请联系系统管理员配置您当前用户的默认部门信息" : "Please contact administrator to assign a company for you!");
                }
            }
        }
        ...
        static public void EnsureDefaultDepartmentExists(ICP.Sys.ServiceInterface.IUserService userService)
        {
            if (string.IsNullOrEmpty(LocalData.UserInfo.DefaultDepartmentName))
            {
                ...
                {
                    throw new Exception(LocalData.IsEnglish ? "请联系系统管理员配置您当前用户的默认公司信息" : "Please contact administrator to assign a company for you!");
                }
            }
        }
        ...
        {
            string message = LocalData.IsEnglish ? "Srue Delete Current Data?" : "是否删除当前数据?";
        ...
        {
            string tip = LocalData.IsEnglish ? "Please Input Code or Name." : "请输入代码或名称.";
            Utility.SetTextEditNullValuePrompt(textEdits, tip);
        }
        ...
        /// </summary>
        /// <param name="userService"></param>
        public static List<UserList> GetUserList(ICP.Sys.ServiceInterface.IUserService userService)
        {
        ...
        /// </summary>
        public static Guid[] GetCompanyIDs(ICP.Sys.ServiceInterface.IUserService userService)
        {
            ...
            if (userCompanyList != null && userCompanyList.Count > 0)
            {
                foreach (var item in userCompanyList)
                {
                    if (!companyIDs.Contains(item.ID))
                    {
        ...
        /// <returns></returns>
        public static Guid[] GetDepartmentIDs(Guid userID, ICP.Sys.ServiceInterface.IUserService userService)
        {

            List<Guid> depIDList = new List<Guid>();

            List<OrganizationList> userDepList = userService.GetUserCompanyList(LocalData.UserInfo.LoginID, OrganizationType.Department);
            if (userDepList.Count == 0)
            {
                throw new Exception(LocalData.IsEnglish ? "You have no rights to query data of any company. Please contat administrator." : "您没有权限查询任何口岸公司的数据，请联系管理员！");
            }
            foreach (var item in userDepList)
            {
                if (item.Type == OrganizationType.Department)
                {
                    depIDList.Add(item.ID);
                }
            }


            return depIDList.ToArray();
        }
        /// <summary>
        /// 获取全部公司办事处列表
        /// </summary>
        ...
        /// <returns></returns>
        public static List<OrganizationList> GetAllCompanyList(IOrganizationService orgService)
        {
        ...
        ///  获得当前用户的公司列表
        /// </summary>
        public static List<OrganizationList> GetCompanyList(ICP.Sys.ServiceInterface.IUserService userService)
        {
            List<OrganizationList> userCompanyList = userService.GetUserCompanyList(LocalData.UserInfo.LoginID, OrganizationType.Company);
        ...
        #region 根据列表数据源打印PDF
        public static void PrintAllDispatchDocument(List<OceanBusinessDownLoadList> downloadlist)
        {
            ...
            if (downloadlist.Exists(aa => aa.DownLoadState != DownLoadState.Downloaded))
            {
                LocalCommonServices.ErrorTrace.SetErrorInfo(null, LocalData.IsEnglish ? "Can not contain a file is not Download, please check! !" : "不能包含未下载的文件，请检查！");
                return;
            }
            ...
            List<DocumentInfo> listdocuments = new List<DocumentInfo>();
            List<ContentInfo> contents = new List<ContentInfo>();
            List<BusinessOperationContext> Contextlist = new List<BusinessOperationContext>();
            string filesPath = "";
            ...
            try
            {
                ...
                foreach (Process p in allProcess1)
                {
                    ...
                    if (p.ProcessName.ToLower() == "acrord32")
                    {
                ...
                downloadlist.ForEach(b =>
                {
                    if (b.RefID != Guid.Empty)
                    {
                        listguid.Add(b.RefID);
                    }
                });

                foreach (Guid item in listguid)
                {
                    Contextlist.Add(CreateBusinessOperationContext(item));
                }
                listdocuments = ClientFileService.GetBusinessDocumentList(Contextlist);
                listdocuments.ForEach(b => fileid.Add(b.Id));

                if (fileid.Count == 0)
                {
                    LocalCommonServices.ErrorTrace.SetErrorInfo(null, LocalData.IsEnglish ? "No file can be merged !" : "没有已签收的文件可以合并!");
                    return;
                }
                contents = FileService.GetDocumentCopyContents(fileid);


                filesPathlist = DataCacheUtility.SaveFileListToDisk(contents);
                //将多个PDF合并成一个

                filesPath = FileConvertService.MergePDFFiles(filesPathlist);

                if (File.Exists(filesPath))
                {
                    CommonUIUtility.OpenWith(filesPath);
                }
            }
            catch(Exception ex)
            {
                ICP.Framework.CommonLibrary.LogHelper.SaveLog("ICPDebug","Message:"+ex.Message);
                ICP.Framework.CommonLibrary.LogHelper.SaveLog("ICPDebug","StackTrace:"+ex.StackTrace);
            }
        ...
        private static BusinessOperationContext CreateBusinessOperationContext(Guid OperationID)
        {
            BusinessOperationContext context = new BusinessOperationContext {OperationID = OperationID};
            context.Add("DocumentState", DocumentState.Accepted);
            return context;
        }
