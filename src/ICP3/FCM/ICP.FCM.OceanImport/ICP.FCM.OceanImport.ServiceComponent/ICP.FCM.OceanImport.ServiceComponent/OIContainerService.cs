...
namespace ICP.FCM.OceanImport.ServiceComponent
{
    partial class OceanImportService 
    {
        ...
        private List<OIBusinessContainerList> ConvertTableToOIBusinessContainerList(DataTable dt)
        {
            if (dt == null || dt.Rows.Count <= 0)
            {
                return new List<OIBusinessContainerList>();
            }
            List<OIBusinessContainerList> results = (from b in dt.AsEnumerable()
                                                     select new OIBusinessContainerList
                                                     {
                                                         ...
                                                         DeliveryTime = b.Field<DateTime?>("DeliveryTime"),
        ...
        public List<OIBusinessContainerList> GetOIContainerList(Guid ioBookingID)
        {
            try
            {
                ...
                return ConvertTableToOIBusinessContainerList(ds.Tables[0]);
        ...
        public ManyResult SaveOIContainerInfo(ContainerSaveRequest saveRequest)
        {
            try
            {
                ...
                string DeliveryTimeList = saveRequest.DeliveryTimes.Join();
                ...
                db.AddInParameter(dbCommand, "@DeliveryTimes", DbType.String, DeliveryTimeList);
        ...
        #region 根据海进业务ID获得海进文件比较时海出业务的集装箱列表
        /// <summary>
        /// 获得海进文件比较时海出业务的集装箱列表
        /// 2013-07-08 joe
        /// </summary>
        /// <param name="ioBookingID">业务单ID</param>
        /// <returns></returns>
        public List<OIBusinessContainerList> GetOICompareContainerList(Guid ioBookingID)
        {
            try
            {
                Database db = DatabaseFactory.CreateDatabase();
                DbCommand dbCommand = db.GetStoredProcCommandWithTimeout("[fcm].[uspGetCompareContainerInfo]");

                db.AddInParameter(dbCommand, "@BookingId", DbType.Guid, ioBookingID);
                db.AddInParameter(dbCommand, "@IsEnglish", DbType.Boolean, isEnglish);
                ...
                List<OIBusinessContainerList> results = (from b in ds.Tables[0].AsEnumerable()
                                                         select new OIBusinessContainerList
                                                         {
                                                             ...
                                                             ID = b.Field<Guid>("ID"),
                                                             No = b.Field<String>("No"),
                                                             ContainerTypeID = b.Field<Guid>("typeid"),
                                                             ContainerTypeName = b.Field<String>("DESCRIPTION"),
                                                             SealNo = b.Field<String>("sealno"),
                                                            // GODate = b.Field<DateTime?>("GODate"),
                                                             //BLNo = b.Field<String>("BLNO"),
                                                             Quantity = b.Field<Int32>("Quantity"),
        ...
        /// <returns></returns>
        public List<OIBusinessContainerList> GetDispatchContainerInfo(Guid OperationID, Guid DispatchFileLogID)
        {
            try
            {
                Database db = DatabaseFactory.CreateDatabase();
                DbCommand dbCommand = db.GetStoredProcCommandWithTimeout("[fcm].[uspGetDispatchContainerInfo]");
