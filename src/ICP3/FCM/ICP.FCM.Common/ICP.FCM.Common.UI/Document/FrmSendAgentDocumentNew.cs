...
namespace ICP.FCM.Common.UI.Document
{
    ...
    /// 
    /// </summary>
    public partial class FrmSendAgentDocumentNew : BaseEditPart, IDisposable
    {
        #region 服务注入
        [ServiceDependency]
        ...
        #region 构造函数
        public FrmSendAgentDocumentNew()
        {
            InitializeComponent();
        ...
        private bool ValidateData()
        {
            ...
            {
                SetLabelMessage(Color.Red, LocalData.IsEnglish ? "Please Input correct email address." : "请输入正确的邮件地址.");
                return false;
            }
        ...
        public void SendEmail(MailOIBusinessDataObjects mailInfo)
        {
            try
            {
                ...
                SingleResult sr = _messageService.Send(message);
            }
            catch (Exception ex)
            {
