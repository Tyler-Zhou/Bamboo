...
namespace ICP.FCM.OceanExport.UI
{
    ...
    /// 海运运价查询参数类
    /// </summary>
    public sealed class FreightPriceQueryParameter
    {
        ...
        DateTime startDate = DateTime.SpecifyKind(DateTime.Now.AddDays(-7), DateTimeKind.Unspecified);
        DateTime endDate = DateTime.SpecifyKind(DateTime.Now.AddDays(7), DateTimeKind.Unspecified);
    ...
    public class OEUtility
    {
        ...
        /// 
        /// </summary>
        static ICP.Sys.ServiceInterface.IUserService UserService
        {
            get
            {
        ...
        /// <param name="lwImageComboBoxEdit"></param>
        public static void BindEnumItems<T>(ICP.Framework.ClientComponents.Controls.LWImageComboBoxEdit lwImageComboBoxEdit) where T : struct, IConvertible
        {
            BindEnumItems<T>(lwImageComboBoxEdit, false);
        }
        /// <summary>
        /// 给下拉框绑定枚举
        /// </summary>
        /// <typeparam name="T">枚举类型，其中对应"未知"的枚举项的名称必须为Unknown(不区分大小写)</typeparam>
        /// <param name="lwImageComboBoxEdit">需要绑定枚举值的下拉框</param>
        /// <param name="removeUnknown">是否需要移除"Unkown"枚举项</param>
        public static void BindEnumItems<T>(ICP.Framework.ClientComponents.Controls.LWImageComboBoxEdit lwImageComboBoxEdit, bool removeUnknown) where T : struct, IConvertible
        {
            BindEnumItems<T>(lwImageComboBoxEdit, removeUnknown, false);
        }
        /// <summary>
        /// 给下拉框绑定枚举
        /// </summary>
        ...
        /// <param name="lwImageComboBoxEdit">需要绑定枚举值的下拉框</param>
        /// <param name="removeUnknown">是否需要移除"Unkown"枚举项</param>
        /// <param name="addEmptyItem">是否需要添加空选择项</param>
        public static void BindEnumItems<T>(ICP.Framework.ClientComponents.Controls.LWImageComboBoxEdit lwImageComboBoxEdit, bool removeUnknown, bool addEmptyItem) where T : struct, IConvertible
        {
            BindEnumItems<T>(lwImageComboBoxEdit, removeUnknown, addEmptyItem, false);
        }
        /// <summary>
        /// 给下拉框绑定枚举
        /// </summary>
        ...
        /// <param name="removeUnknown">是否需要移除"Unkown"枚举项</param>
        /// <param name="addEmptyItem">是否需要添加空选择项</param>
        /// <param name="addAllItem">是否添加一个全选选项</param>
        public static void BindEnumItems<T>(ICP.Framework.ClientComponents.Controls.LWImageComboBoxEdit lwImageComboBoxEdit, bool removeUnknown, bool addEmptyItem, bool addAllItem)
        {
            if (addAllItem && addEmptyItem)
            {
                throw new ICPException("不允许同时添加空和全选选项!");
            }
            List<ICP.Framework.CommonLibrary.Helper.EnumHelper.ListItem<T>> enumItems = ICP.Framework.CommonLibrary.Helper.EnumHelper.GetEnumValues<T>(LocalData.IsEnglish);
            if (removeUnknown)
            {
                enumItems.RemoveAll(item => item.Value.ToString().ToLower() == "unknown");
            }
            lwImageComboBoxEdit.Properties.BeginUpdate();
            lwImageComboBoxEdit.Properties.Items.Clear();
            if (addEmptyItem)
            {
                lwImageComboBoxEdit.Properties.Items.Add(new DevExpress.XtraEditors.Controls.ImageComboBoxItem(string.Empty, DBNull.Value));
            }
            if (addAllItem)
            {
                lwImageComboBoxEdit.Properties.Items.Add(new DevExpress.XtraEditors.Controls.ImageComboBoxItem(string.Empty, DBNull.Value));
            ...
            {
                lwImageComboBoxEdit.Properties.Items.Add(new DevExpress.XtraEditors.Controls.ImageComboBoxItem(item.Name, item.Value));
            }
            lwImageComboBoxEdit.Properties.EndUpdate();
        }
        /// <summary>
        /// 
        /// </summary>
        ...
        #region 设置文档中心数据源
        public static void SetDocumentListDataSource(UCDocumentList documentList, DocumentListPresenter documentPresenter, MemoParam para)
        {
            documentPresenter.ucList = documentList;
            BusinessOperationContext context = BuildContext(para);
            documentPresenter.BindData(context);

        }

        #endregion

        #region Fax/Mail/EDI沟通历史记录
        ...
        public static decimal GetRate(string sourceCurrency, string targetCurrency, DateTime billdate, List<ICP.Common.ServiceInterface.DataObjects.SolutionExchangeRateList> rateList)
        {
            decimal rate = 1m;
        ...
        public static decimal GetRate(Guid sourceCurrencyID, Guid targetCurrencyID, DateTime billdate, List<ICP.Common.ServiceInterface.DataObjects.SolutionExchangeRateList> rateList)
        {
            decimal rate = 1m;

            if (sourceCurrencyID == targetCurrencyID || sourceCurrencyID == Guid.Empty || targetCurrencyID == Guid.Empty)
                return rate;

            var rateobj = rateList.Find(delegate(ICP.Common.ServiceInterface.DataObjects.SolutionExchangeRateList r)
            {
                return r.SourceCurrencyID == sourceCurrencyID &&
            ...
            if (rateobj == null)
            {
                rateobj = rateList.Find(delegate(ICP.Common.ServiceInterface.DataObjects.SolutionExchangeRateList r)
                {
                    return r.TargetCurrencyID == sourceCurrencyID &&
                            r.SourceCurrencyID == targetCurrencyID &&
                            billdate.Date >= r.FromDate &&
                            billdate.Date <= r.ToDate;
                });
        ...
        public static decimal GetAmountByRate(decimal amount, Guid sourceCurrencyID, Guid tagerCurrencyID, List<SolutionExchangeRateList> rateList)
        {
            if (sourceCurrencyID == Guid.Empty || tagerCurrencyID == Guid.Empty) return 0m;

            if (sourceCurrencyID == tagerCurrencyID) return amount;

            SolutionExchangeRateList inRate = rateList.Find(delegate(SolutionExchangeRateList item)
            { return item.SourceCurrencyID == sourceCurrencyID && item.TargetCurrencyID == tagerCurrencyID; });
            if (inRate != null)
            {
            ...
            { return item.SourceCurrencyID == tagerCurrencyID && item.TargetCurrencyID == sourceCurrencyID; });
            if (outRate != null)
            {
                return (amount / outRate.Rate);
            }

            string sourceCurrencyName = rateList.Find(delegate(SolutionExchangeRateList item)
            { return item.SourceCurrencyID == sourceCurrencyID; }).SourceCurrency;

            string tagerCurrencyName = rateList.Find(delegate(SolutionExchangeRateList item)
            { return item.SourceCurrencyID == tagerCurrencyID; }).SourceCurrency;
        ...
        public static object[] GetSingleSearchResult<T>(T data, string[] returnFields)
        {
            ...
            {
                result[i] = OEUtility.GetObjectPropertyStringValue<T>(data, returnFields[i]);
            }
        ...
        public static object[] GetMultiSearchResult<T>(List<T> datas, string[] returnFields)
        {
            ...
            {
                result[i] = OEUtility.GetSingleSearchResult<T>(datas[i], returnFields);
            }
            return result;
        }
        ...
        #region 收发通代理人的XMLHelper

        public static string BuindDescriptionFromXml(CustomerDescriptionForNew customerDesc, bool isContainsName, bool isAddAddressStr)
        {
            StringBuilder strb = new StringBuilder();
            try
            {
                bool hasOtherInfo = false;
                if (isContainsName)
                {
                    if (!string.IsNullOrEmpty(customerDesc.Name.Trim()))
                    {
                        strb.Append(ConvertXmlBack(customerDesc.Name.Trim()) + "\r\n");
                        hasOtherInfo = true;
                    }
                }
                ...
                bool hasCCInfo = false;

                if (!string.IsNullOrEmpty(customerDesc.Address.Trim()))
                {
                    strb.Append((isAddAddressStr ? "Address:" : string.Empty) + ConvertXmlBack(customerDesc.Address.Trim()));
                    hasCCInfo = hasOtherInfo = true;
                }
                if (!string.IsNullOrEmpty(customerDesc.City.Trim()))
                {
                    strb.Append(" " + ConvertXmlBack(customerDesc.City.Trim()));
                    hasCCInfo = hasOtherInfo = true;
                }
                if (!string.IsNullOrEmpty(customerDesc.Country.Trim()))
                {
                    if (hasCCInfo)
                        strb.Append("," + ConvertXmlBack(customerDesc.Country.Trim() + " ."));
                    else
                        strb.Append(ConvertXmlBack(customerDesc.Country.Trim() + "."));
                    hasCCInfo = hasOtherInfo = true;
                }
                if (hasCCInfo) strb.Append("\r\n");

                #endregion

                #region Contact

                bool hasTelorFax = false;
                if (!string.IsNullOrEmpty(customerDesc.Tel.Trim()))
                {
                    strb.Append("Tel:" + ConvertXmlBack(customerDesc.Tel.Trim()));
                    hasTelorFax = hasOtherInfo = true;
                }
                if (!string.IsNullOrEmpty(customerDesc.Fax.Trim()))
                {
                    if (!string.IsNullOrEmpty(customerDesc.Fax)) strb.Append(" ");

                    strb.Append("Fax:" + ConvertXmlBack(customerDesc.Fax.Trim()));
                    hasTelorFax = hasOtherInfo = true;
                }
                if (hasTelorFax) strb.Append("\r\n");

                if (!string.IsNullOrEmpty(customerDesc.Contact.Trim()))
                {
                    strb.Append(ConvertXmlBack(customerDesc.Contact.Trim()) + "\r\n");
                    hasOtherInfo = true;
                }

                #endregion

                if (!string.IsNullOrEmpty(customerDesc.Remark))
                {
                    strb.Append(ConvertXmlBack(customerDesc.Remark.Trim()));
                }

                return strb.ToString().ToUpper();
            }
            catch { return strb.ToString().ToUpper(); }
        }

        public static string BuindDescriptionFromXml(CustomerDescription customerDesc, bool isContainsName, bool isAddAddressStr)
        {
            StringBuilder strb = new StringBuilder();
            try
            {
        ...
        public static string ParseXmlForCustomerDesToOther(string strXml)
        {
            string xml = string.Empty;
            xml += "<CustomerDescription>";
            xml += "<Name></Name>";
            xml += "<Address></Address>";
            xml += "<Country></Country>";
            xml += "<City></City>";
            ...
            xml += "<Tel></Tel>";
            xml += "<Contact></Contact>";
            xml += "<Ramark>" + strXml.ToString() + "</Ramark>";
            xml += "</CustomerDescription>";
            return xml;
        }
        ...
        {
            return ConvertXml(xml, true);

        }
        /// <summary>
        /// XML转义字符处理,isPass,是否过滤<,>,
        /// </summary>
        public static string ConvertXml(string xml, bool isPass)
        {
            xml = xml.Replace("<?xml version=\"1.0\"?>", "");//去掉板本号
        ...
        /// 把XML特殊字符还原
        /// </summary>
        public static string ConvertXmlBack(string xml)
        {
            xml = xml.Replace("&amp;", "&");
            xml = xml.Replace("&lt;", "<");
            xml = xml.Replace("&gt;", ">");
        ...
        public static void BatRegisterPortFinder(List<DevExpress.XtraEditors.ButtonEdit> controls, IDataFindClientService dfService, bool isEnglish)
        {
            foreach (DevExpress.XtraEditors.ButtonEdit ctl in controls)
            {
                OEUtility.SetEnterToExecuteOnec(ctl, delegate
                {
                    PortFinderBridge pfbPlaceOfReceipt = new PortFinderBridge(ctl, dfService, isEnglish);
                });
            }
        }
        ...
        /// 加拿大
        /// </summary>
        public static List<Guid> CSShippingLines
        {
            get
            {
                List<Guid> idlist = new List<Guid>();
        ...
        /// <summary>
        /// 根据OceanBLContainerList生成箱号
        /// </summary>
        /// <param name="ctnList">OceanBLContainerList</param>
        /// <returns>箱号</returns>
        public static string BulidCtnNOByContainerList(List<OceanBLContainerList> ctnList)
        {
            if (ctnList == null || ctnList.Count == 0) return string.Empty;
        ...
        /// <param name="ctnList">OceanBLContainerList</param>
        /// <returns>箱号</returns>
        public static string BulidCtnNOByDeclareContainerList(List<DeclareBLContainerList> ctnList)
        {
            if (ctnList == null || ctnList.Count == 0) return string.Empty;
            StringBuilder strBuilder = new StringBuilder();
            foreach (var item in ctnList)
        ...
        public static void SetCustomerTextEditNullValuePrompt(List<DevExpress.XtraEditors.TextEdit> textEdits)
        {
            ...
            OEUtility.SetTextEditNullValuePrompt(textEdits, tip);
        }
        ...
        /// </summary>
        /// <returns></returns>
        public static string GetXmlValueForKey(String key)
        {
            string fileRootDirectory = Path.Combine(System.AppDomain.CurrentDomain.BaseDirectory, "BusinessTemplates");
            string tempalteFileName = "BookingConfirmationConfig.xml";
            string filePath = Path.Combine(fileRootDirectory, tempalteFileName);
            string value = "";

            XmlDocument document = new XmlDocument();
            document.Load(filePath);

            String path = @"//ShippingLineConfig/ShippingLineRemark/add[@key='" + key + "']";
            XmlNodeList xmlAdds = document.SelectNodes(path);
    ...
    public class UIModelHelper
    {
        ...
        public static void SetContainerItem(DevExpress.XtraEditors.Repository.RepositoryItemImageComboBox cmbEdit)
        {
            cmbEdit.Items.Add(new DevExpress.XtraEditors.Controls.ImageComboBoxItem("20GP", Guid.NewGuid()));
            cmbEdit.Items.Add(new DevExpress.XtraEditors.Controls.ImageComboBoxItem("40GP", Guid.NewGuid()));
            cmbEdit.Items.Add(new DevExpress.XtraEditors.Controls.ImageComboBoxItem("40HQ", Guid.NewGuid()));
            cmbEdit.Items.Add(new DevExpress.XtraEditors.Controls.ImageComboBoxItem("45HQ", Guid.NewGuid()));
        }
    }
}
