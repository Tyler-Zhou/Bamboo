...
namespace ICP.FCM.DomesticTrade.UI
{
    public partial class POItemEditPart : XtraUserControl, IChildPart
    {
        ...
        private void DeleteData()
        {
            ...
            {
                //if (IsOrderPO)
                    DomesticTradeService.RemoveDTOrderPOInfo(CurrentRow.ID, LocalData.UserInfo.LoginID, CurrentRow.UpdateDate,LocalData.IsEnglish);
                //else
        ...
        public void InitData(Guid orderId)
        {
            ...
            {                
                OrderId = orderId;
                List<DTBookingPOList> _DTBookingPOInfo = DataSource as List<DTBookingPOList>;
                if (_DTBookingPOInfo == null)
                {
                    _DTBookingPOInfo = DomesticTradeService.GetDTOrderPOList(OrderId,LocalData.IsEnglish);
                    ...
                    SetSource(_DTBookingPOInfo);
                }
            }
            else
            {
                SetSource(new List<DTBookingPOList>());
            }
        }

        void item_PropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            List<DTBookingPOList> pos = DataSource as List<DTBookingPOList>;
            if (pos == null)
            {
                return ;
            }
