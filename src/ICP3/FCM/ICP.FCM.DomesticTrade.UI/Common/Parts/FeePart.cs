...
namespace ICP.FCM.DomesticTrade.UI.Common
{
    ...
    public partial class FeePart : XtraUserControl, IChildPart
    {
         ...
         DTBookingFeeList CurrentRow
        {
            get
            {
                if (bsFee.Current == null)
                    ...
                    return bsFee.Current as DTBookingFeeList;
            }
        }

         List<DTBookingFeeList> SelectRows
         {
             get
             {
                 ...
                 List<DTBookingFeeList> list = new List<DTBookingFeeList>();
                 foreach (var item in indexs)
                 {
                     DTBookingFeeList tager = gridViewFee.GetRow(item) as DTBookingFeeList;
        ...
        public decimal GetProfit(Guid CurrencyID)
        {
            ...
            if (ArgumentHelper.GuidIsNullOrEmpty(CurrencyID) || _rateList == null || _rateList.Count == 0)
            {
                string message = LocalData.IsEnglish?"Have no rate at current company.":"找不到当前公司下的汇率.";
                throw new ApplicationException(message);
            }
            List<DTBookingFeeList> fee = bsFee.DataSource as List<DTBookingFeeList>;
            ...
            {
                if (ArgumentHelper.GuidIsNullOrEmpty(item.CurrencyID)) continue;
        ...
        public string GetDRAmountString()
        {
            ...
            fee = fee.FindAll(delegate(DTBookingFeeList item) { return item.Way == FeeWay.AR; });
        ...
        public string GetCRAmountString()
        {
            ...
            fee = fee.FindAll(delegate(DTBookingFeeList item) { return item.Way == FeeWay.AP; });
            if (fee == null || fee.Count == 0)
            {
                return ZeroMoney;
            }
        ...
        private string GetAmountString( List<DTBookingFeeList> fee)
        {
            string amountString = string.Empty;
            ...
            foreach (var item in fee)
            {
                if (ArgumentHelper.GuidIsNullOrEmpty(item.CurrencyID) || item.Amount <= 0) continue;
        ...
        {
            DTBookingFeeList preRow=null;
            if (gridViewFee.RowCount > 0)
                preRow = gridViewFee.GetRow(gridViewFee.RowCount - 1) as DTBookingFeeList;

            DTBookingFeeList newFeeRow;

            if (preRow != null)
                newFeeRow = Utility.Clone<DTBookingFeeList>(preRow);
            else
                newFeeRow = new DTBookingFeeList();
            ...
            if(ArgumentHelper.GuidIsNullOrEmpty(newFeeRow.CurrencyID))
            ...
            gridViewFee.ClearSorting();
            (bsFee.List as List<DTBookingFeeList>).Add(newFeeRow);
        ...
        private void barDelete_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    DomesticTradeService.RemoveDTOrderFeeList(needRemoveIDs.ToArray(),_CompanyID, LocalData.UserInfo.LoginID, needRemoveUpdateDate.ToArray());
                }
            ...
            catch (Exception ex) { LocalCommonServices.ErrorTrace.SetErrorInfo(FindForm()
                ,( LocalData.IsEnglish ? "Delete Faily" : "删除失败.")+ex.Message); }
        }

        private void gridViewFee_CellValueChanged(object sender, CellValueChangedEventArgs e)
        {
            //Amount
            if (e.Column.Name == colUnitPrice.Name || e.Column.Name == colQuantity.Name)
            {
                ...
                if (row != null && row as DTBookingFeeList != null)
                {
                    DTBookingFeeList fee = row as DTBookingFeeList;
        ...
        public bool IsChanged
        {
            get
            {
                if (_isChanged == false)
                {
                    List<DTBookingFeeList> source = bsFee.DataSource as List<DTBookingFeeList>;
                    if (source != null)
                    {
        ...
        public bool ValidateData()
        {
            ...
            foreach (var item in bsFee.DataSource as List<DTBookingFeeList>)
            {
                if (item.Validate
                    (
        ...
        {
            get { return bsFee.DataSource as List<DTBookingFeeList>; }
        }
