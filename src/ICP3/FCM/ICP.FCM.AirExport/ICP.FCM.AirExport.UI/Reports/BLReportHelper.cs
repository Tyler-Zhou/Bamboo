...
namespace ICP.FCM.AirExport.UI
{
    public class BLReportHelper
    {
        ...
        [ServiceDependency]
        public IAirExportService AirExportService { get; set; }
        /// <summary>
        /// 提单报表打印
        /// </summary>
        ...
        public Dictionary<string, object> Print(BLReportData_HBL HBL, Guid Id, bool isEnglish, int type, string[] reportPara)
        {
            ...
            if (type == 0) { data = AirExportService.GetHBLReportData(Id, isEnglish); }
            else {data = AirExportService.GetHBLReportData(Id, isEnglish); }
