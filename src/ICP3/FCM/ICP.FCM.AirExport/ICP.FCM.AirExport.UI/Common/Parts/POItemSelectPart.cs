...
namespace ICP.FCM.AirExport.UI
{
    public partial class POItemSelectPart : XtraUserControl, IChildPart
    {
        ...
        public POItemSelectPart()
        {
            ...
            gridViewItem.ShownEditor += new EventHandler(gridViewItem_ShownEditor);

            gridViewItem.CalcRowHeight += new RowHeightEventHandler(gridViewItem_CalcRowHeight);
        ...
        void gridViewItem_CalcRowHeight(object sender, RowHeightEventArgs e)
        {
        }
        ...
        public void SetAllPO(Guid orderId, WorkItem workItem, Guid containerId)
        {
            if (_orderId == Guid.Empty)
            {
                ...
                CompositePoAndItems pos = AirExportService.GetAirContainerPOList(orderId);

                _pos = pos;

                foreach (AirBookingPOList po in _pos.PoList)
                {
        ...
        public List<POSaveRequest> CollectPosData()
        {
            ...
            if (_pos != null && _pos.PoList.Count > 0)
            {
                if (IsChanged)
                {
                    ...
                    List<AirBookingPOList> _listHasChanged = _pos.PoList.FindAll(o => o.IsDirty);
        ...
        void item_PropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            ...
            //{
                AirBookingPOList po = _pos.PoList.First(o => o.ID == item.POID);
                if (po != null)
                {
                    po.IsDirty = true;
                }
        ...
        private void DeleteData()
        {
            ...
            if (gridControl1.FocusedView == gridViewPO)
            {
                ...
                {
                    AirExportService.RemoveAirContainerPOInfo(CurrentRow.ID, LocalData.UserInfo.LoginID, CurrentRow.UpdateDate);
                }
