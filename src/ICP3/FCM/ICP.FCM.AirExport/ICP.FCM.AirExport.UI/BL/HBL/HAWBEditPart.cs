...
{
    [ToolboxItem(false)]
    public partial class HAWBEditPart : BaseEditPart
    {
        #region Service
        ...
        public HAWBEditPart()
        {
            ...
            SetNullValuePrompt();
            Disposed += delegate {
                SmartPartClosing -= MAWBEditPart_SmartPartClosing;
                _airMBLs = null;
                _countryList = null;
                _CurrentBLInfo = null;
                ...
                Saved = null;
                OtherChargesBindingSource.DataSource = null;
                OtherChargesBindingSource.Dispose();
                txtChLBS.TextChanged -= txtChLBS_TextChanged;
        ...
        /// 主体数据
        /// </summary>
        AirHBLInfo _CurrentBLInfo = null;
        ...
        /// 原始提单号
        /// </summary>
        string _originalHBLNO = string.Empty;

        const decimal KGS_TO_LBS_QUOTIETY = 2.2046M;

        List<BookingBLInfo> _airMBLs = null;
        ...
        private void InitControls()
        {
            ...
            barDelect.Glyph = GlobalResource.Delete_16;
            RefreshToolBars();
        ...
        {
            if (!ArgumentHelper.GuidIsNullOrEmpty(_CurrentBLInfo.CompanyID))
            {
        ...
        void SetComboboxEnumSource()
        {
            ...
            //运价等级
            List<EnumHelper.ListItem<RateClass>> rateClasss = EnumHelper.GetEnumValues<RateClass>(LocalData.IsEnglish);
            foreach (var rateItem in rateClasss)
            {
                if (rateItem.Value == 0) continue;
                cmbTarifflevel.Properties.Items.Add(new ImageComboBoxItem(rateItem.Name, rateItem.Value));
            }
        }
        ...
        {
            mcmbAirCompany.ShowSelectedValue(_CurrentBLInfo.AirCompanyID, _CurrentBLInfo.AirCompanyName);
            ...
            {
                List<DataDictionaryList> payments = ICPCommUIHelper.SetCmbDataDictionary(cmbOther, DataDictionaryType.PaymentTerm, DataBindType.EName);
            });
            ...
            if (_CurrentBLInfo.IsNew == false || _CurrentBLInfo.AirOrderID.IsNullOrEmpty() == false)
            {
                AirBookingList booking = AirExportService.GetAirBookingListByIds(new Guid[] { _CurrentBLInfo.AirBookingID })[0];
                _airMBLs = booking.AirMBLs;
                foreach (var item in _airMBLs)
                {
        ...
        void SearchRegister()
        {
            ...
            //	查询面板及栏位定义，请参照订舱单列表
            //	筛选范围：订舱单.状态不等于“已关单”
            DataFindClientService.Register(stxtRefNo, FCMFinderConstants.AirBookingFinder, SearchFieldConstants.BookingNO, SearchFieldConstants.AirBookingResultValue,
                  ...
                  delegate
                  {
                      ...
                      stxtRefNo.Tag = _CurrentBLInfo.AirBookingID = Guid.Empty;
            ...
            #region POL
            DataFindClientService.Register(txtDepartureCode, CommonFinderConstants.AirLocationFinder, SearchFieldConstants.CodeName, SearchFieldConstants.PortResultValue,
                  delegate(object inputSource, object[] resultData)
                  {
            ...
            #endregion
            #region POD
            DataFindClientService.Register(txtDetinationCode, CommonFinderConstants.AirLocationFinder, SearchFieldConstants.CodeName, SearchFieldConstants.PortResultValue,
                  delegate(object inputSource, object[] resultData)
                  {
                      Guid portID = new Guid(resultData[0].ToString());
            ...
            #region IssuePlace
            DataFindClientService.Register(stxtIssuePlace, CommonFinderConstants.AirLocationFinder, SearchFieldConstants.CodeName, SearchFieldConstants.PortResultValue,
                delegate(object inputSource, object[] resultData)
                {
        ...
        /*删除其它费用*/
        private void barDelect_ItemClick(object sender, ItemClickEventArgs e)
        {
            DialogResult dlg = XtraMessageBox.Show(LocalData.IsEnglish ? "Are you sure to delete?" : "确认删除当前费用?", LocalData.IsEnglish ? "Confirm" : "确认", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
        ...
        {
            #region 	如果之前已选择业务号
            if (ArgumentHelper.GuidIsNullOrEmpty(_CurrentBLInfo.AirBookingID) == false && _CurrentBLInfo.AirBookingID != bookingID)
            {
            ...
            AirBookingInfo bookingInfo = AirExportService.GetAirBookingInfo(bookingID);
            #region 如果选择的订舱单为“只出MBL”时，提示用户，需要修改订单属性，然后再新增HBL。

            if (bookingInfo.IsOnlyMBL)
            {
                if (XtraMessageBox.Show(LocalData.IsEnglish ? "Un Done" : "此订舱单已钩选\"只出MBL\",强制关联HBL会自动修改订舱单数据.是否继续?"
                                    , LocalData.IsEnglish ? "Tip" : "提示"
                                    , MessageBoxButtons.YesNo
                                    , MessageBoxIcon.Question) == DialogResult.No)
                {
            ...
            _CurrentBLInfo.AirBookingID = bookingInfo.ID;
            ...
            if (bookingInfo.HBLReleaseType != null && bookingInfo.HBLReleaseType.Value != FCMReleaseType.Unknown)
                _CurrentBLInfo.ReleaseType = bookingInfo.HBLReleaseType.Value;
            ...
            _CurrentBLInfo.ShipperDescription = bookingInfo.ShipperDescription;
            if (_CurrentBLInfo.ShipperDescription != null)
                txtShipperDescription.Text = _CurrentBLInfo.ShipperDescription.ToString(LocalData.IsEnglish);
            ...
            //_CurrentBLInfo.PlaceOfReceiptID = bookingInfo.PlaceOfReceiptID;
            //_CurrentBLInfo.PlaceOfReceiptCode = bookingInfo.PlaceOfReceiptName;
            //_CurrentBLInfo.PlaceOfReceiptName = bookingInfo.PlaceOfReceiptName;
            ...
            ////{
            ////    _CurrentBLInfo.FinalDestinationID = bookingInfo.PlaceOfDeliveryID;
            ////    _CurrentBLInfo.FinalDestinationCode = bookingInfo.PlaceOfDeliveryName;
            ////    _CurrentBLInfo.FinalDestinationName = bookingInfo.PlaceOfDeliveryName;
            ////}
            ////else
            ////{
            ////    _CurrentBLInfo.FinalDestinationID = bookingInfo.FinalDestinationID;
            ////    _CurrentBLInfo.FinalDestinationCode = bookingInfo.FinalDestinationName;
            ////    _CurrentBLInfo.FinalDestinationName = bookingInfo.FinalDestinationName;
            ////}
            ...
            //_CurrentBLInfo.TransportClauseID = bookingInfo.TransportClauseID;
            //_CurrentBLInfo.TransportClauseName = bookingInfo.TransportClauseName;
            //cmbTransportClause.ShowSelectedValue(_CurrentBLInfo.TransportClauseID, _CurrentBLInfo.TransportClauseName);

            cmbPaymentTerm.SelectedIndexChanged -= new EventHandler(cmbPaymentTerm_SelectedIndexChanged);
            _CurrentBLInfo.BookingPaymentTermID = _CurrentBLInfo.PaymentTermID = bookingInfo.HBLPaymentTermID;
            cmbPaymentTerm.Text = _CurrentBLInfo.PaymentTermName = bookingInfo.HBLPaymentTermName;
            cmbPaymentTerm.ShowSelectedValue(_CurrentBLInfo.PaymentTermID, _CurrentBLInfo.PaymentTermName);
            ...
            //if (ICP.Framework.CommonLibrary.Helper.ArgumentHelper.GuidIsNullOrEmpty(bookingInfo.WeightUnitID) == false)
            //{
            //    cmbWeightUnit.Text = _CurrentBLInfo.WeightUnitName = bookingInfo.WeightUnitName;
            //    _CurrentBLInfo.WeightUnitID = bookingInfo.WeightUnitID.Value;
            ...
            //bookingContainerDescription = bookingInfo.ContainerDescription;
            bindingSource1.EndEdit();

            cmbMBLNO.Properties.Items.Clear();
            _airMBLs = bookingInfo.AirMBLs;
            if (_airMBLs == null) _airMBLs = new List<BookingBLInfo>();
            foreach (var item in _airMBLs)
            {
                cmbMBLNO.Properties.Items.Add(item.NO);
            }
        ...
        private void cmbFlightNo_SelectedRow(object sender, EventArgs e)
        {
            if (cmbFlightNo.EditValue != null && cmbFlightNo.EditValue.ToString().Length > 0)
            {
                ...
                //dteETD.EditValue = _CurrentBLInfo.ETD = flightInfo.ETD;
                //dteETA.EditValue = _CurrentBLInfo.ETA = flightInfo.ETA;
        ...
        //private void cmbFlightNo_Enter(object sender, EventArgs e)
        //{
        //    if ((Guid)this.txtDepartureCode.Tag == Guid.Empty) { MessageBoxService.ShowInfo(LocalData.IsEnglish ? "Please select POL at first." : "请先选择起运港！"); return; }
        //    if ((Guid)this.txtDetinationCode.Tag == Guid.Empty) { MessageBoxService.ShowInfo(LocalData.IsEnglish ? "Please select POD at first." : "请先选择目的港！"); return; }

        //    try
        //    {
        //        List<FlightList> flightList = TransportFoundationService.GetFlightList(null, null, txtDepartureCode.Text, txtDetinationCode.Text, null, null, null, null, null, null, true, 100);
        //        List<FlightListHBL> flightListClient = new List<FlightListHBL>();
        //        if (flightList != null && flightList.Count > 0)
        //        {
        //            foreach (FlightList r in flightList)
        //            {
        //                FlightListHBL tmp = new FlightListHBL();
        //                ICP.Framework.CommonLibrary.Helper.ObjectHelper.CopyData(r, tmp);
        //                flightListClient.Add(tmp);
        //            }

        //            Dictionary<string, string> col = new Dictionary<string, string>();
        //            col.Add("No", "航班号");
        //            col.Add("AirlineName", "航空公司");
        //            col.Add("ClosingDateString", "截关日");
        //            cmbFlightNo.InitSource<FlightListHBL>(flightListClient, col, "No", "ID");
        //        }
        //        else
        //        {
        //            MessageBoxService.ShowInfo(LocalData.IsEnglish ? "No corresponding flight." : "没有找到对应的航班号！");
        //        }
        //    }
        //    catch (Exception ex)
        //    {
        ...
        //private void ChangeFlightNoToNull()
        //{
        //    cmbFlightNo.EditValue = _CurrentBLInfo.FilightNoID = null;
        //    cmbFlightNo.EditText = _CurrentBLInfo.FilightNo = string.Empty;
        //}
        ...
        void PlaceOfDeliveryChanged()
        {
                ...
                && _CurrentBLInfo.IsNew == false)
            {
                try
                {
                    bool isExist = AirExportService.IsPortCountryExistCompanyConfig(_CurrentBLInfo.PlaceOfDeliveryID.Value, _CurrentBLInfo.CompanyID);
                    if (isExist == false)
                    {
                        if (XtraMessageBox.Show(LocalData.IsEnglish ? "Current BL need request agent.is clear current BL\'s agent?"
                                                                  : "当前提单需要申请代理,是否清空代理?"
                           , LocalData.IsEnglish ? "Tip" : "提示"
                           , MessageBoxButtons.YesNo
        ...
        void cmbPaymentTerm_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                if (XtraMessageBox.Show(LocalData.IsEnglish ? "Un Done" : "现选择的HBL付款方式与订舱单中HBL付款方式不同,是否继续?",
                                             LocalData.IsEnglish ? "Tip" : "提示",
                                             MessageBoxButtons.YesNo,
                                             MessageBoxIcon.Question) == DialogResult.No)
                {
        ...
        private bool Save()
        {
            ...
            try
            {
                ...
                BookingBLInfo existBlItem = _airMBLs.Find(delegate(BookingBLInfo item) { return item.NO == _CurrentBLInfo.MBLNo; });
                if (existBlItem != null)
                {
                ...
                else if (ArgumentHelper.GuidIsNullOrEmpty(_CurrentBLInfo.MBLID) == false)
                {
                    ...
                    {
                        BookingBLInfo mbl = _airMBLs.Find(delegate(BookingBLInfo item) { return item.ID == _CurrentBLInfo.MBLID; });
                ...
                SingleResult resultData = AirExportService.SaveAirHBLInfo(hbl);
                UpdateBLBySaved(resultData);
                AfterSave(false);
                isSave = true;
        ...
        private void UpdateBLBySaved(SingleResult result)
        {
            ...
            _CurrentBLInfo.MBLID = result.GetValue<Guid>("AirMBLID");
            _CurrentBLInfo.No = result.GetValue<string>("No");
            _CurrentBLInfo.UpdateDate = result.GetValue<DateTime?>("UpdateDate");
            _CurrentBLInfo.MBLUpdateDate = result.GetValue<DateTime?>("MWBUpdateDate");

            BookingBLInfo existBlItem = _airMBLs.Find(delegate(BookingBLInfo item) { return item.ID == _CurrentBLInfo.MBLID; });
            if (existBlItem != null)
            {
        ...
        void AfterSave(bool IsSaveAs)
        {
            ...
            stxtRefNo.Properties.ReadOnly = true;
            stxtRefNo.Properties.Buttons[0].Enabled = false;
            Title = LocalData.IsEnglish ? "Edit HAWB " + _CurrentBLInfo.No : "编辑HAWB " + _CurrentBLInfo.No;
            RefreshBarEnabled();
            if (Saved != null) Saved(new object[] { _CurrentBLInfo });

            if (IsSaveAs)
                LocalCommonServices.ErrorTrace.SetSuccessfullyInfo(FindForm(), LocalData.IsEnglish ? "Save as a new bl successfully." : "已成功另存为一票新的提单.");
            else
        ...
        bool SaveAs()
        {
            ...
            if (XtraMessageBox.Show(LocalData.IsEnglish ? "Un Done" : "是否另存为一票新的提单?",
                            LocalData.IsEnglish ? "Tip" : "提示",
                            MessageBoxButtons.YesNo,
                            MessageBoxIcon.Question) == DialogResult.No)
            {
            ...
            try
            {
                ...
                SingleResult result = AirExportService.SaveAirHBLInfo(mbl);
        ...
        void MAWBEditPart_SmartPartClosing(object sender, WorkspaceCancelEventArgs e)
        {
        ...
        private Message.ServiceInterface.Message GetOperationInfo()
        {
            ...
            message.UserProperties.OperationId = _CurrentBLInfo.AirBookingID;
        ...
        private void barBill_ItemClick(object sender, ItemClickEventArgs e)
        {
            using (new CursorHelper(Cursors.WaitCursor))
            {
                ...
                OperationCommonInfo operationCommonInfo = FCMCommonService.GetOperationCommonInfo(_CurrentBLInfo.AirBookingID, OperationType.AirExport);
        ...
        private void barSubCheck_ItemClick(object sender, ItemClickEventArgs e)
        {
            using (new CursorHelper(Cursors.WaitCursor))
            {
                ...
                try
                {
                    SingleResult result = AirExportService.ChangeAirHBLState(_CurrentBLInfo.ID, AEBLState.Checked, LocalData.UserInfo.LoginID, _CurrentBLInfo.UpdateDate);
                    bool isDirty = _CurrentBLInfo.IsDirty;
                    _CurrentBLInfo.ID = result.GetValue<Guid>("ID");
                    _CurrentBLInfo.UpdateDate = result.GetValue<DateTime?>("UpdateDate");
                    _CurrentBLInfo.State = AEBLState.Checked;
        ...
        {
            bool srcc = FCMCommonClientService.OpenAgentRequestPart(_CurrentBLInfo.ID, OperationType.AirExport,null,null);
            if (srcc)
            {
                bool isDirty = _CurrentBLInfo.IsDirty;
                _CurrentBLInfo.IsRequestAgent = true;
        ...
        private void RefreshBLData()
        {
            ...
            //{
            Focus();
            _CurrentBLInfo = AirExportService.GetAirHBLInfo(_CurrentBLInfo.ID);
            _CurrentBLInfo.CancelEdit();
            _CurrentBLInfo.BeginEdit();
        ...
        {
            txtGKGS.Enter += delegate(object sender, EventArgs e) { txtGKGS.TextChanged += new EventHandler(txtGKGS_TextChanged); };
            txtGLBS.Enter += delegate(object sender, EventArgs e) { txtGLBS.TextChanged += new EventHandler(txtGLBS_TextChanged); };
            txtChKGS.Enter += delegate(object sender, EventArgs e) { txtChKGS.TextChanged += new EventHandler(txtChKGS_TextChanged); };
            txtChLBS.Enter += delegate(object sender, EventArgs e) { txtChLBS.TextChanged += new EventHandler(txtChLBS_TextChanged); };

            txtGKGS.Leave += delegate(object sender, EventArgs e) { txtGKGS.TextChanged -= new EventHandler(txtGKGS_TextChanged); };
            txtGLBS.Leave += delegate(object sender, EventArgs e) { txtGLBS.TextChanged -= new EventHandler(txtGLBS_TextChanged); };
            txtChKGS.Leave += delegate(object sender, EventArgs e) { txtChKGS.TextChanged -= new EventHandler(txtChKGS_TextChanged); };
            txtChLBS.Leave += delegate(object sender, EventArgs e) { txtChLBS.TextChanged -= new EventHandler(txtChLBS_TextChanged); };

            txtBuyPrice.Enter += delegate(object sender, EventArgs e) { txtBuyPrice.TextChanged += new EventHandler(txtBuyPrice_TextChanged); };
            radioGroup1.Enter += delegate(object sender, EventArgs e) { radioGroup1.EditValueChanged += new EventHandler(txtBuyPrice_TextChanged); };

            txtBuyPrice.Leave += delegate(object sender, EventArgs e) { txtBuyPrice.TextChanged -= new EventHandler(txtBuyPrice_TextChanged); };
            radioGroup1.Leave += delegate(object sender, EventArgs e) { radioGroup1.EditValueChanged -= new EventHandler(txtBuyPrice_TextChanged); };
        }
        ...
        private void txtBuyPrice_TextChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    salesamount = ConvertToDecimal(txtChKGS.Text);
                }
                else
                {
                    salesamount = ConvertToDecimal(txtChLBS.Text);
                }

                _CurrentBLInfo.RateCharge = ConvertToDecimal(txtBuyPrice.Text);
                txtBuyAmount.EditValue = _CurrentBLInfo.RateAmount = ConvertToDecimal(txtBuyPrice.Text) * salesamount;
            }
            catch (Exception ex)
            {
                LocalCommonServices.ErrorTrace.SetErrorInfo(FindForm(), ex);
        ...
        private void txtGKGS_TextChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                txtGKGS.TextChanged -= new EventHandler(txtGKGS_TextChanged);

                txtGLBS.Text = (ConvertToDecimal(txtGKGS.Text) * KGS_TO_LBS_QUOTIETY).ToString("F3");
        ...
        private void txtGLBS_TextChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                txtGLBS.TextChanged -= new EventHandler(txtGLBS_TextChanged);

                txtGKGS.Text = (ConvertToDecimal(txtGLBS.Text) / KGS_TO_LBS_QUOTIETY).ToString("F3");
                _CurrentBLInfo.GrossKGS = Convert.ToDecimal(txtGKGS.Text);
        ...
        private void txtChKGS_TextChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                txtChKGS.TextChanged -= new EventHandler(txtChKGS_TextChanged);

                txtChLBS.Text = (ConvertToDecimal(txtChKGS.Text) * KGS_TO_LBS_QUOTIETY).ToString("F3");
        ...
        private void txtChLBS_TextChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                txtChLBS.TextChanged -= new EventHandler(txtChLBS_TextChanged);

                txtChKGS.Text = (ConvertToDecimal(txtChLBS.Text) / KGS_TO_LBS_QUOTIETY).ToString("F3");
        ...
        {
            _CurrentBLInfo = data as AirHBLInfo;
            bindingSource1.DataSource = _CurrentBLInfo;
            ...
            RefreshBarEnabled();
            InitCustomerDescriptionObject();
            _originalHBLNO = _CurrentBLInfo.No;
            //_CurrentBLInfo.BeginEdit();
        }
        ...
        SaveHBLInfoParameter ConvertHBLToParameter(bool IsSaveAs, AirHBLInfo info)
        {
            ...
            if (info.OtherChargeList != null && info.OtherChargeList.Count > 0)
            {
                ...
                otherChargersXMLString = SerializerHelper.SerializeToString<OtherChargeXML>(item, true, true);
            }
