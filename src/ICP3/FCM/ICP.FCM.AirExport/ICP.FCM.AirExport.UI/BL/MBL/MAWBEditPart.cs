...
namespace ICP.FCM.AirExport.UI.MBL
{
    ...
    public partial class MAWBEditPart : BaseEditPart
    {
        ...
        {
            base.OnLoad(e);
            SmartPartClosing += new EventHandler<WorkspaceCancelEventArgs>(MAWBEditPart_SmartPartClosing);
        ...
        void cmbPaymentTerm_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                if (XtraMessageBox.Show(LocalData.IsEnglish ? "Un Done" : "现选择的MBL付款方式与订舱单中MBL付款方式不同,是否继续?",
                                             LocalData.IsEnglish ? "Tip" : "提示",
                                             MessageBoxButtons.YesNo,
                                             MessageBoxIcon.Question) == DialogResult.No)
                {
        ...
        private bool Save()
        {
            ...
            try
            {
                ...
                SingleResult result = AirExportService.SaveAirMBLInfo(mbl);
        ...
        void AfterSave(bool IsSaveAs)
        {
            ...
            stxtRefNo.Properties.ReadOnly = true;
            stxtRefNo.Properties.Buttons[0].Enabled = false;
            Title = LocalData.IsEnglish ? "Edit MAWB " + _CurrentBLInfo.No : "编辑MAWB " + _CurrentBLInfo.No;
            RefreshBarEnabled();
        ...
        //    try
        //    {
        //        SingleResult result = AirExportService.ChangeAirMBLState(_CurrentBLInfo.ID, BLState.Checking, LocalData.UserInfo.LoginID, _CurrentBLInfo.UpdateDate);
        ...
        private void barSubCheck_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            try
            {
                SingleResult result = AirExportService.ChangeAirMBLState(_CurrentBLInfo.ID, AEBLState.Checked, LocalData.UserInfo.LoginID, _CurrentBLInfo.UpdateDate);
        ...
        private void RefreshBLData()
        {
            ...
            //{
            Focus();
            _CurrentBLInfo = AirExportService.GetAirMBLInfo(_CurrentBLInfo.ID);
            _CurrentBLInfo.CancelEdit();
            _CurrentBLInfo.BeginEdit();
        ...
        private void textboxEventControl()
        {
            ...
            txtIATAPrice.Enter += delegate(object sender, EventArgs e) { txtIATAPrice.TextChanged += new EventHandler(txtIATABuyPrice_TextChanged); };
            radioGroup2.Enter += delegate(object sender, EventArgs e) { radioGroup2.EditValueChanged += new EventHandler(txtIATABuyPrice_TextChanged); };
            ...
            txtIATAPrice.Leave += delegate(object sender, EventArgs e) { txtIATAPrice.TextChanged -= new EventHandler(txtIATABuyPrice_TextChanged); };
            radioGroup2.Leave += delegate(object sender, EventArgs e) { radioGroup2.EditValueChanged -= new EventHandler(txtIATABuyPrice_TextChanged); };
        }
        ...
        private void txtIATABuyPrice_TextChanged(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    iaamount = ConvertToDecimal(txtChKGS.Text);
                }
                else
                {
                    iaamount = ConvertToDecimal(txtChLBS.Text);
                }

                _CurrentBLInfo.IATARateCharge = ConvertToDecimal(txtIATAPrice.Text);
                txtIATAAmount.EditValue = _CurrentBLInfo.IATARateAmount = ConvertToDecimal(txtIATAPrice.Text) * iaamount;
            }
            catch (Exception ex)
            {
                LocalCommonServices.ErrorTrace.SetErrorInfo(FindForm(), ex);
