...
namespace ICP.FCM.AirExport.ServiceComponent
{
    partial class AirExportService
    {
        ...
        /// <returns></returns>
        public BLReportData_HBL GetMBLReportData(Guid Id, bool isEnglish, AWBType type)
        {
            ...
            if (type == AWBType.HAWB) { result.HAWBNO = ds.Tables[0].Rows[0]["HAWBNO"].ToString(); }
        ...
        /// <returns></returns>
        public AEOrderReportData GetAEOrderReportData(Guid bookID,Guid companyID)
        {
            try
            {
                Database db = null;
