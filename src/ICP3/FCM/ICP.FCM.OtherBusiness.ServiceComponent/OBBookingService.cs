...
namespace ICP.FCM.OtherBusiness.ServiceComponent
{
    ...
    partial class OtherBusinessService 
    {
        ...
        public Dictionary<Guid, SaveResponse> SaveOtherBusinessWithTrans(OtherBusinessSaveRequest saveRequest,
            List<FeeSaveRequest> fees, List<ContainerSaveRequest> _container, List<SaveRequestBookingDelegate> delegates)
        {
            ...
            using (TransactionScope scope = new TransactionScope(TransactionScopeOption.Required, option))
            {
                ...
                if (saveRequest != null)
                {
                    result.Add(saveRequest.RequestId,
                        new SaveResponse { RequestId = saveRequest.RequestId, SingleResult = SaveOtherBusinessInfo(saveRequest) });
                ...
                if (fees != null)
                {
                    foreach (FeeSaveRequest fee in fees)
                    {
                        ...
                        result.Add(fee.RequestId,
                            new SaveResponse { RequestId = fee.RequestId, ManyResult = SaveOBOrderFeeList(fee) });
                    }
                }
                if (_container != null)
                {
                    foreach (ContainerSaveRequest con in _container)
                    {
                        ...
                        result.Add(con.RequestId,
                            new SaveResponse { RequestId = con.RequestId, ManyResult = SaveOtherContanerList(con) });
                    }
                }
                if (delegates != null)
                {
                    foreach (SaveRequestBookingDelegate cspDelegate in delegates)
                    {
                        if (saveRequest != null && saveRequest.id == Guid.Empty)
                        {
            ...
            int maxRecords)
        {
            try
            {
                ...
                dds = GetOtherBusinessDataSet(companyIDs, departIDs, otOperationTypes, no, MBLNo, HBLNo, ContainerNo,
                    Consignee, Shipper, NotifyPart, customerName, polName, podName, placeOfDeliveryName,
                    VesselName, VoyageNo, isValid, orderState, salesID, overseasFilerID, dateSearchType,
                    beginTime, endTime, maxRecords, isDefaultDB == false ? null : (bool?)true);
                sds = GetOtherBusinessDataSet(companyIDs, departIDs, otOperationTypes, no, MBLNo, HBLNo, ContainerNo,
                    Consignee, Shipper, NotifyPart, customerName, polName, podName, placeOfDeliveryName,
                    VesselName, VoyageNo, isValid, orderState, salesID, overseasFilerID, dateSearchType,
                    beginTime, endTime, maxRecords, isDefaultDB == true ? null : (bool?)false);
                ...
                List<OtherBusinessList> results = BulidOrderListByDataSet(dsResult);
        ...
        private DataSet GetOtherBusinessDataSet(Guid[] companyIDs,
            Guid[] departIDs,
            int[] otOperationTypes,
            string no,
            string MBLNo,
            ...
            bool? isDefaultDB)
        {
            ...
            string tempDepartIDs = departIDs.Join();//待之后处理/(订单/其他业务）
            string tempOTOperationTypes = otOperationTypes.Join();
            ...
            db.AddInParameter(dbCommand, "@OTOperationTypes", DbType.String, tempOTOperationTypes);
        ...
        /// </summary>
        /// <param name="companyIDs">公司列表</param>
        /// <param name="OTOperationTypes">其他业务类型(1:本地业务;2:代订舱;3:其他业务;4:指定货;5:FBA空派;6:FBA海派)</param>
            ...
            int maxRecords)
        {
            try
            {
                ...
                dds = GetOtherBusinessDataSetForFaster(companyIDs, OTOperationTypes, noSearchType, no, customerSearchType, customerName,
                    portSearchType, portName, dateSearchType, salesId, OperatorID, beginTime, endTime, isValid, maxRecords
                    , true);
                sds = GetOtherBusinessDataSetForFaster(companyIDs, OTOperationTypes, noSearchType, no, customerSearchType, customerName,
                    portSearchType, portName, dateSearchType, salesId, OperatorID, beginTime, endTime, isValid, maxRecords
                    , false);
                dsResult = DataSetHelper.MergeSet(dds, sds);
                if (dsResult == null || dsResult.Tables.Count < 1)
        ...
        private DataSet GetOtherBusinessDataSetForFaster(Guid[] companyIDs,
            int[] OTOperationTypes,
            NoSearchType noSearchType,
            string no,
            CustomerSearchType customerSearchType,
        ...
        /// <param name="companyIDs">订单口岸ID集合</param>
        /// <returns>返回订单列表</returns>
        public List<OtherBusinessList> GetOtherBusinessListById(Guid[] orderIDs, Guid[] companyIDs)
        {
            ...
            try
            {
                ...
                bool? isDefaultDB = CompanyHelper.IsDefaultServer(companyIDs, FrameworkInitializeService);
                dds = GetOtherBusinessDataSetById(orderIDs, isDefaultDB == false ? null : (bool?)true);
                sds = GetOtherBusinessDataSetById(orderIDs, isDefaultDB == true ? null : (bool?)false);
                dsResult = DataSetHelper.MergeSet(dds, sds);
                if (dsResult == null || dsResult.Tables.Count < 1)
                {
                    return null;
        ...
        private DataSet GetOtherBusinessDataSetById(Guid[] orderIDs, bool? IsDefaultDB)
        {
            if (IsDefaultDB == null)
                return null;
            Database db = DatabaseFactoryHelper.CreateDatabase(IsDefaultDB.Value);
        ...
        public OtherBusinessInfo GetOtherBusinessInfo(Guid id, Guid companyID)
        {
            ...
            try
            {
                ...
                return ConvertDataSetToBusinessInfo(ds);
            }
            catch (SqlException sqlException)
            {
                throw new ApplicationException(sqlException.Message);
        ...
        private OtherBusinessInfo ConvertDataSetToBusinessInfo(DataSet ds)
        {
            OtherBusinessInfo result = (from b in ds.Tables[0].AsEnumerable()
                                        select new OtherBusinessInfo
                                        {
                                            ...
                                            CostAmount = b.Field<decimal>("Cost"),
                                            RevenueTon = b.Field<decimal>("RevenueTon"),
        ...
        public SingleResult CancelOtherBusiness(Guid orderID, bool isCancel, Guid changeByID, DateTime? updateDate)
        {
        ...
        /// <param name="updateDate">更新时间-做数据版本用</param>
        /// <returns>返回SingleResult</returns>
        public SingleResult CancelOtherBusiness(Guid orderID, Guid companyID, bool isCancel, Guid changeByID, DateTime? updateDate)
        {
            ArgumentHelper.AssertGuidNotEmpty(orderID, "ID");
            ArgumentHelper.AssertGuidNotEmpty(changeByID, "removeByID");
        ...
        public SingleResult SaveOtherBusinessInfo(OtherBusinessSaveRequest saveRequest)
        {
            try
            {
                ...
                db.AddInParameter(dbCommand, "@ExpressNo", DbType.String, saveRequest.ExpressNo);
                ...
                db.AddInParameter(dbCommand, "@CostAmount", DbType.Decimal, saveRequest.CostAmount);
                db.AddInParameter(dbCommand, "@RevenueTon", DbType.Decimal, saveRequest.RevenueTon);
                ...
                if (saveRequest != null && saveRequest.IsSyncCSP && !saveRequest.SONO.IsNullOrEmpty())
                {
                    SaveRequestShipmentInfoForCSP sqShipmentInfo = new SaveRequestShipmentInfoForCSP()
                    {
                        ...
                        SaveBy = saveRequest.SaveByID.Value,
        ...
        /// <returns></returns>
        /// 
        public ProfitContainerObjects GetOtherProfitReportData(Guid OperationID, Guid companyID)
        {
            ArgumentHelper.AssertGuidNotEmpty(OperationID, "OperationID");
            try
            {
