...
namespace ICP.FCM.OtherBusiness.UI
{
    ...
    /// UI辅助类
    /// </summary>
    public class OtherUIHelper : Controller,IDisposable
    {
        #region Services
            ...
            bool ensureFirstEmpty)
        {
            ...
            if (ensureFirstEmpty)
            {
                if (list.FindAll(item => ICP.Framework.CommonLibrary.Helper.ArgumentHelper.GuidIsNullOrEmpty(item.ID)).Count == 0)
                {
                    foreach (var cmbEdit in cmbEdits)
                    {
            ...
            bool ensureFirstEmpty)
        {
            ...
            if (ensureFirstEmpty)
            {
                if (users.FindAll(item => ICP.Framework.CommonLibrary.Helper.ArgumentHelper.GuidIsNullOrEmpty(item.ID)).Count == 0)
                {
        ...
        /// <param name="customerDescriptions">客户的描述,不可为空</param>
        ...
        public void ShowTruckEdit(Guid id, OtherBusinessList CurrentRow, WorkItem workItem, IOtherBusinessService obService, string no)
        {
            ...
            PartLoader.ShowEditPart<TruckEditPart>(workItem, truckList, stateValues, title + no, null,
                OBBCommandConstants.Command_Truck + id.ToString());
        }

        #region IDisposable 成员
        public void Dispose()
        {
            this._dataDictionarys = null;
    ...
    /// 打印辅助类
    /// </summary>
    public class OtherBusinessPrintHelper
    {
        #region Services
        ...
        public IReportViewer PrintOBOrder(Guid orderID, Guid companyID)
        {
            OBOrderReportData data = OBReportSrvice.GetOBOrderReportData(orderID, companyID);
        ...
        /// 打印利润表
        /// </summary>
        /// <param name="otherBusinessList">bookingID</param>
        /// <returns>返回IReportViewer</returns>
        public IReportViewer PrintOEBookingProfit(OtherBusinessList otherBusinessList)
        {
            ...
            ProfitContainerObjects data = OBService.GetOtherProfitReportData(_otherBusinessList.ID, _otherBusinessList.CompanyID);
