...
{
    public partial class TreeCheckBox : DevExpress.XtraEditors.XtraUserControl
    {
        #region init
        ...
        private List<TreeCheckBoxSource> BulidDataSource<T>(List<T> source, string displayMember, string canCheckedProperty)
        {
            ...
            List<TreeCheckBoxSource> topTss = treeSources.FindAll(delegate(TreeCheckBoxSource item) { return ICP.Framework.CommonLibrary.Helper.ArgumentHelper.GuidIsNullOrEmpty(item.ParentID); });
