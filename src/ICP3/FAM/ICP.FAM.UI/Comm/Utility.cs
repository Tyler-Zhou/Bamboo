...
namespace ICP.FAM.UI
{
    ...
    public class FAMUtility
    {
        ...
        /// 当前登陆用户默认所属公司名称
        /// </summary>
        static string defaultCompanyName = string.Empty;
        /// <summary>
        /// 当前登陆用户默认所属公司ID
        /// </summary>
        static Guid defaultCompanyId = Guid.Empty;
        /// <summary>
        /// 当前登陆用户默认所属部门名称
        /// </summary>
        static string defaultDepartmentName = string.Empty;
        /// <summary>
        /// 当前登陆用户默认所属部门ID
        /// </summary>
        static Guid defaultDepartmentId = Guid.Empty;
        /// <summary>
        /// 当前用户所属公司列表
        /// </summary>
        static List<LocalOrganizationInfo> userCompanyList = LocalData.UserInfo.UserOrganizationList.FindAll(item => item.Type == LocalOrganizationType.Company);
        /// <summary>
        /// 所有公司列表
        /// </summary>
        //static  List<OrganizationList> orgList;
       ...
       public static IConfigureService ConfigureService
       {
        ...
        static public void EnsureDefaultCompanyExists()
        {
            ...
            if (string.IsNullOrEmpty(LocalData.UserInfo.DefaultCompanyName))
            {
                ...
                if (list.Count > 0)
                {
                    defaultCompanyId = LocalData.UserInfo.DefaultCompanyID = list[0].ID;
                    defaultCompanyName = LocalData.UserInfo.DefaultCompanyName = LocalData.IsEnglish ? list[0].EShortName : list[0].CShortName;
                }
                else
                {
        ...
        /// </summary>
        /// <param name="userService"></param>
        static public void EnsureDefaultDepartmentExists()
        {
            ...
            if (string.IsNullOrEmpty(LocalData.UserInfo.DefaultDepartmentName))
            {
                ...
                if (list.Count > 0)
                {
                    defaultDepartmentId = LocalData.UserInfo.DefaultDepartmentID = list[0].ID;
                    defaultDepartmentName = LocalData.UserInfo.DefaultDepartmentName = LocalData.IsEnglish ? list[0].EShortName : list[0].CShortName;
                }
                else
                {
        ...
        /// </summary>
        public static object GetObjectPropertyStringValue<T>(T t, string propertyname)
        {
        ...
        public static object[] GetSingleSearchResult<T>(T data, string[] returnFields)
        {
            ...
            {
                result[i] = GetObjectPropertyStringValue<T>(data, returnFields[i]);
            }
        ...
        //public static bool ValidateContainerNumber(string containerNo)
        //{
        //    if (string.IsNullOrEmpty(containerNo)) return false;
        ...
        //    int equalVal = 0;
        //    double computeNo = 0;
        //    if (CheckLength(ContainerNo) == false) return false;
        //    bool isTure = true;
        ...
        //            if (input != 'U')
        //            {
        //                DialogResult result;//UNDONE translate
        //                messageinfo = (LocalData.IsEnglish  ? "Container No " : "箱号")
        //                               + ContainerNo
        //                               + (LocalData.IsEnglish ?" The Fourth Letters Should \"U\",Is Accepted?":"第4位应为字母\"U\",是否接受此箱号?");

        //                result = DevExpress.XtraEditors.XtraMessageBox.Show(messageinfo,
        ...
        //        if (i <= 3)
        //        {
        //            if (Char.IsUpper(input) == false)
        //            {
        //                isTure = false;//UNDONE translate
        //                messageinfo = (LocalData.IsEnglish ? "Container No " : "箱号")
        //                                + ContainerNo + " "
        //                                + (LocalData.IsEnglish ? "NO" : "号") + (i + 1).ToString() + " "
        //                                + (LocalData.IsEnglish ? "should Capital letters!" : "位应为大写字母!");

        //                DevExpress.XtraEditors.XtraMessageBox.Show(messageinfo,
        //                                LocalData.IsEnglish  ? "Tip" : "提示",
        ...
        //        if (i > 3)
        //        {
        //            if (Char.IsNumber(input) == false)
        //            {
        //                isTure = false;
        //                messageinfo = (LocalData.IsEnglish  ? "Container No " : "箱号")
        //                                + ContainerNo + " "
        //                                + (LocalData.IsEnglish ? "NO" : "号") + (i + 1).ToString() + " "
        //                                + (LocalData.IsEnglish ? "should Num!" : "位应为数字!"); 
        ...
        //            messageinfo = (LocalData.IsEnglish  ? "Container No " : "箱号")
        //                                     + ContainerNo + " "
        //                                     + (LocalData.IsEnglish ? "Not International container No Rules,This container No last one  Check Code should \"":"不符合国际集装箱号编号规则,该箱号最后一位校验码应为\"")
        //                                     + ModNo.ToString()
        //                                     + (LocalData.IsEnglish ? "\",Is Accepted?\"" : ",\"是否接受箱号?\"");
        ...
        //private static bool CheckLength(string ContainerNo)
        //{
        ...
        //        messageinfo = (LocalData.IsEnglish  ? "Container No " : "箱号")
        //                        + ContainerNo
        //                        + (LocalData.IsEnglish  ? " Length < 11,Is Accepted?" : "箱号小于11位,是否接受?");
        ...
        //        messageinfo = (LocalData.IsEnglish  ? "Container No " : "箱号")
        //                                + ContainerNo
        //                                + (LocalData.IsEnglish  ? " Length > 11,Is Accepted?" : "箱号大于11位,是否接受?");
        ...
        public static bool EnquireIsDeleteCurrentData()
        {
            ...
            DialogResult result = XtraMessageBox.Show(LocalData.IsEnglish ? "Srue Delete Current Data?" : "是否删除当前数据?",
                                                 LocalData.IsEnglish ? "Tip" : "提示",
                                                 MessageBoxButtons.YesNo,
                                                 MessageBoxIcon.Question);
            if (result == DialogResult.Yes)
        ...
        /// <summary>
        /// 弹出一个提示"增值税是否以手动更改的为准"
        /// </summary>
        /// <returns></returns>
        public static DialogResult SaveVATchanges() 
        {
            DialogResult result = XtraMessageBox.Show(LocalData.IsEnglish ? "VAT have to manually change, whether manually change the shall prevail?" : "增值税有手动更改，是否以手动更改为准?"
                                    , LocalData.IsEnglish ? "Tip" : "提示"
                                    , MessageBoxButtons.YesNoCancel
                                    , MessageBoxIcon.Question);
            return result;     
        ...
        public static decimal Get2Round(decimal number)
        {
            return decimal.Round(number, 2, MidpointRounding.AwayFromZero);
        }
        public static decimal Get3Round(decimal number)
        {
        ...
        {
            List<LocalOrganizationInfo> list = GetCompanyList();
            ...
            foreach (var item in list)
            {
                ...
                {
                    checkComboBox.Properties.Items.Add(item.ID, name, CheckState.Unchecked, true);
                }
            }
        ...
        /// </summary>
        /// <param name="comBox"></param>
        /// <param name="isAddtAll"></param>
        /// <param name="isSetDefault"></param>
        public static void BindComboBoxByCompany(ImageComboBoxEdit comBox,bool isAddtAll=true,bool isSetDefault=false)
        {
            List<LocalOrganizationInfo> list = GetCompanyList();
            comBox.Properties.BeginUpdate();
            comBox.Properties.Items.Clear();
            ...
            foreach (var item in list)
            {
                ...
                if(isSetDefault && item.ID == LocalData.UserInfo.DefaultCompanyID)
                {
                    comBox.EditValue = item.ID;
                }
            }
            comBox.Properties.EndUpdate();
        }
        ...
        ///  获得当前用户的公司列表
        /// </summary>
        public static List<LocalOrganizationInfo> GetCompanyList()
        {
        ...
        /// <returns></returns>
        public static List<UserList> SetMcmbUsers(MultiSearchCommonBox mcmbUsers, List<Guid> companyIDs, string roleName, string jobName)
        {
        ...
        /// <param name="companyID">公司ID</param>
        /// <returns></returns>
        public static void SetCmbBoxUsers(ImageComboBoxEdit comBox, string roleName, string jobName)
        {
            List<LocalOrganizationInfo> orgList = GetCompanyList();
        ...
        public static void BindComboBoxByAllCompany(ImageComboBoxEdit comBox)
        {

            IOrganizationService orgService = ServiceClient.GetService<IOrganizationService>();
        ...
        /// 绑定指定部门的所有用户
        /// </summary>
        public static void BindCmbBoxUserByOrg(MultiSearchCommonBox mcmbUsers, Guid orgID)
        {
        ...
        public static void BindCmbBoxUser( MultiSearchCommonBox mcmbUsers)
        {
            List<UserList> users = UserService.GetUserListBySearch(Guid.Empty, string.Empty, string.Empty, true, true, 0);
        ...
        /// <returns></returns>
        public static List<SolutionCurrencyList> GetCompanyCurrencyList(Guid companyID)
        {
        ...
        /// <param name="companyID"></param>
        /// <param name="configureService"></param>
        public static List<SolutionGLCodeList> GetGLCodeList(Guid companyID)
        {
            ConfigureInfo configureInfo = ConfigureService.GetCompanyConfigureInfo(companyID);
        ...
        /// <param name="value"></param>
        /// <returns></returns>
        public static Int32 GetIntByDataRow(object value)
        {
            if (value == null||value==DBNull.Value)
            {
                return 0 ;
        ...
        /// <returns></returns>
        public static decimal GetDecimalByDataRow(object value)
        {
            if (value == null||value==DBNull.Value)
            {
                return 0.0m;
        ...
        public static void ShowDispacthReviseResult(int result, WorkItem workItem, Guid operationId)
        {
            switch (result)
            {
                case 1:
                    if (DialogResult.Yes == XtraMessageBox.Show(LocalData.IsEnglish ? "Saving can't be executed. Because D/C Fees have been revised by the agent,\t\n  you must accept the revised fees first. \t\n  Clicks Yes to enter the page [Accept Revised D/C Fees From The Agent] \t\n  Clicks No to continue to enter the page [Account Info]"
                    ...
                    break;
                case 2:
                    ShowMessage(LocalData.IsEnglish ? "Saving the D/C fees  is successful. You must re-dispatch Docs to the agent in time."
                          : "您已成功修改了代理账单，您必须及时地向代理手动再次分发文件(Docs)。");
                    break;
                case 3:
                    if (DialogResult.Yes == XtraMessageBox.Show(LocalData.IsEnglish ? "Saving can't be executed. Because  the agent have re-dispatched Docs,\t\n you must accept the dispatching first. \t\n Clicks Yes to enter the page [Accept Dispatched Docs From The Agent] \t\n Clicks No to continue to enter the page [Account Info]"
                       : "代理账单不能被保存，因为代理已经重新分发了文档，您必须先签收此次分发。换行符 单击[Yes]进入签收代理分发文档页面。换行符 单击[NO]继续打开账单页面。",
                       "", MessageBoxButtons.YesNo))
                    {
                        FCM.Common.UI.FCMUIUtility.ShowAcceptedDocumentCompareNew(workItem, Guid.Empty, operationId, false);
                    }
                    break;
                case 4:
    ...
    public class RateHelper
    {
        ...
        public decimal GetRate(string sourceCurrency, string targetCurrency, DateTime billdate, List<SolutionExchangeRateList> rateList)
        {
            decimal rate = 1m;

            if (sourceCurrency == targetCurrency) return rate;

            var rateobj = rateList.Find(delegate(SolutionExchangeRateList r)
            {
            ...
            if (rateobj == null)
            {
                rateobj = rateList.Find(delegate(SolutionExchangeRateList r)
                {
        ...
        /// <returns></returns>
        public decimal GetRate(Guid sourceCurrencyID, Guid targetCurrencyID, DateTime billdate, List<SolutionExchangeRateList> rateList)
        {
            ...
            SolutionExchangeRateList inRate = rateList.Find(delegate(SolutionExchangeRateList item)
            {
                return
                  ...
                  && billdate.Date >= item.FromDate.Date && billdate.Date <= item.ToDate.Date;
            });
            ...
            XtraMessageBox.Show(LocalData.IsEnglish ? "Don't find at time" : "没有找到在时间" + billdate.ToShortDateString() + " " + sourceCurrencyName + "=>" + tagerCurrencyName + (LocalData.IsEnglish ? " Rate Not Find." : " 的汇率."), LocalData.IsEnglish ? "Tip" : "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            //throw new ApplicationException(sourceCurrencyName + "=>" + tagerCurrencyName + (LocalData.IsEnglish ? " Rate Not Find." : " 找不到汇率."));
            return 0m;
        }

        public decimal GetAmountByRate(decimal amount, Guid sourceCurrencyID, Guid targetCurrencyID, List<SolutionExchangeRateList> rateList, DateTime billdate)
        {
            if (sourceCurrencyID == Guid.Empty || targetCurrencyID == Guid.Empty) return 0m;
            ...
            SolutionExchangeRateList inRate = rateList.Find(delegate(SolutionExchangeRateList item)
            { return item.SourceCurrencyID == sourceCurrencyID && item.TargetCurrencyID == targetCurrencyID && billdate.Date >= item.FromDate && billdate.Date <= item.ToDate; });
            if (inRate != null)
            {
            ...
            SolutionExchangeRateList outRate = rateList.Find(delegate(SolutionExchangeRateList item)
            { return item.SourceCurrencyID == targetCurrencyID && item.TargetCurrencyID == sourceCurrencyID && billdate.Date >= item.FromDate && billdate.Date <= item.ToDate; });
            if (outRate != null)
            {
        ...
        #region 通过币种名查找对应的汇率对象(有时间约束)

        /// <summary>
        /// 通过币种名查找对应的汇率对象(有时间约束)
        /// </summary>
        /// <param name="sourceCurrencyID">源币种</param>
        /// <param name="targetCurrencyID">目标币种</param>
        /// <param name="billdate">日期</param>
        /// <param name="rateList">汇率列表</param>
        /// <param name="isNeedNoMatchTip">没找到匹配汇率时是否需要提示</param>
        /// <returns></returns>
        public SolutionExchangeRateList GetMatchSolutionExchangeRateList(string sourceCurrency, string targetCurrency, DateTime billdate, List<SolutionExchangeRateList> rateList, bool isNeedNoMatchTip)
        {
            ...
            {
                if (rateList[i].SourceCurrency == sourceCurrency && rateList[i].TargetCurrency == targetCurrency && billdate.Date >= rateList[i].FromDate && billdate.Date <= rateList[i].ToDate)
                {
                    return rateList[i];
                }
                else if (rateList[i].TargetCurrency == sourceCurrency && rateList[i].SourceCurrency == targetCurrency && billdate.Date >= rateList[i].FromDate && billdate.Date <= rateList[i].ToDate)
                {
                    SolutionExchangeRateList solutionExchangeRate = FAMUtility.Clone<SolutionExchangeRateList>(rateList[i]);
                    solutionExchangeRate.SourceCurrencyID = rateList[i].TargetCurrencyID;
                    solutionExchangeRate.SourceCurrency = rateList[i].TargetCurrency;
                    solutionExchangeRate.TargetCurrencyID = rateList[i].SourceCurrencyID;
                    solutionExchangeRate.TargetCurrency = rateList[i].SourceCurrency;
                    solutionExchangeRate.Rate = 1 / rateList[i].Rate;

                    return solutionExchangeRate;
                }

                if (isNeedNoMatchTip && i == rateList.Count - 1)
                {
                    XtraMessageBox.Show(LocalData.IsEnglish ? "Don't find at time" : "没有找到在时间" + billdate.ToShortDateString() + " " + sourceCurrency + "=>" + targetCurrency + (LocalData.IsEnglish ? " Rate Not Find." : " 的汇率."), LocalData.IsEnglish ? "Tip" : "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
        ...
        /// <param name="isNeedNoMatchTip">没找到匹配汇率时是否需要提示</param>
        /// <returns></returns>
        public SolutionExchangeRateList GetMatchSolutionExchangeRateList(Guid sourceCurrencyID, Guid targetCurrencyID, DateTime billdate, List<SolutionExchangeRateList> rateList, bool isNeedNoMatchTip)
        {
            ...
            {
                SolutionExchangeRateList exchangeRate = new SolutionExchangeRateList();
                exchangeRate.SourceCurrencyID = sourceCurrencyID;
                exchangeRate.SourceCurrency = GetCurrencyNameByCurrencyID(sourceCurrencyID);
            ...
            for (int i = 0; i < rateList.Count; i++)
            {
                if (rateList[i].SourceCurrencyID == sourceCurrencyID && rateList[i].TargetCurrencyID == targetCurrencyID && billdate.Date >= rateList[i].FromDate && billdate.Date <= rateList[i].ToDate)
                {
                    return rateList[i];
                }
                else if (rateList[i].TargetCurrencyID == sourceCurrencyID && rateList[i].SourceCurrencyID == targetCurrencyID && billdate.Date >= rateList[i].FromDate && billdate.Date <= rateList[i].ToDate)
                {
                    SolutionExchangeRateList solutionExchangeRate = FAMUtility.Clone<SolutionExchangeRateList>(rateList[i]);
                    solutionExchangeRate.SourceCurrencyID = rateList[i].TargetCurrencyID;
                    solutionExchangeRate.SourceCurrency = rateList[i].TargetCurrency;
        ...
        /// <param name="rateList">汇率列表</param>
        /// <returns></returns>
        public SolutionExchangeRateList GetMatchSolutionExchangeRateList(Guid sourceCurrencyID, Guid targetCurrencyID, List<SolutionExchangeRateList> rateList, bool isNeedNoMatchTip)
        {
            if (sourceCurrencyID == Guid.Empty || targetCurrencyID == Guid.Empty)
            {
                return null;
            ...
            for (int i = 0; i < rateList.Count; i++)
            {
                if (rateList[i].SourceCurrencyID == sourceCurrencyID && rateList[i].TargetCurrencyID == targetCurrencyID)
                {
                    return rateList[i];
                }
                else if (rateList[i].TargetCurrencyID == sourceCurrencyID && rateList[i].SourceCurrencyID == targetCurrencyID)
                {
                    SolutionExchangeRateList solutionExchangeRate = FAMUtility.Clone<SolutionExchangeRateList>(rateList[i]);
                    solutionExchangeRate.SourceCurrencyID = rateList[i].TargetCurrencyID;
                    solutionExchangeRate.SourceCurrency = rateList[i].TargetCurrency;
                ...
                if (isNeedNoMatchTip && i == rateList.Count - 1)
                {
                    string sourceCurrencyName = GetCurrencyNameByCurrencyID(sourceCurrencyID);
                    string tagerCurrencyName = GetCurrencyNameByCurrencyID(targetCurrencyID);
                    XtraMessageBox.Show(sourceCurrencyName + "=>" + tagerCurrencyName + (LocalData.IsEnglish ? " Rate Not Find." : " 找不到汇率."), LocalData.IsEnglish ? "Tip" : "提示", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
    ...
    public class FAMUIHelper : Controller
    {
        #region Services
    ...
    public class RandomHelper
    {
        ...
        {
            return decimal.Parse((Random.Next(-500, 500) + r.NextDouble()).ToString("n"));
        }
    ...
    public class UIConnectionHelper
    {
        ...
        public static void ParentChangingForEditPart(IListPart listPart, SaveDataDelegate saveData, BaseDataObject childDataSource, CancelEventArgs e, string partName, bool removeMainList)
        {
            ...
            {
                DialogResult dlg = FAMUtility.EnquireIsSaveCurrentDataByNew(partName);
            ...
            {
                DialogResult dlg = FAMUtility.EnquireIsSaveCurrentDataByUpdated(partName);
                if (dlg == DialogResult.Yes) e.Cancel = !saveData();
                else if (dlg == DialogResult.Cancel) e.Cancel = true;
                else if (dlg == DialogResult.No && childDataSource.IsNew && removeMainList)
                {
