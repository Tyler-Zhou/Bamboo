...
namespace ICP.FAM.UI.Comm
{
    public class IListDataSet
    {
        ...
        public static DataSet ToDataSet<T>(IList<T> p_List, params string[] p_PropertyName)
        {
            ...
            if (p_List.Count > 0)
            {
                ...
                foreach (PropertyInfo pi in propertys)
                {
                    ...
                    {
                        // 没有指定属性的情况下全部属性都要转换  
                        if (pi.ToString().Contains("System.Nullable"))
                        {
                            int fstIndex = pi.ToString().IndexOf('[') + 1;
                            int sndIndex = pi.ToString().LastIndexOf(']');
                            
                            _DataTable.Columns.Add(pi.Name, pi.ToString().Substring(fstIndex, sndIndex - fstIndex).GetType());
                        }
                        else
                            _DataTable.Columns.Add(pi.Name, pi.PropertyType);
                    }
        ...
        /// <param name="p_DataSet">DataSet</param>    
        /// <param name="p_TableIndex">待转换数据表索引</param>    
        /// <param name="rowNumber">自增行的字段名</param>
        /// <returns></returns>
        public static IList<T> DataSetToIList<T>(DataSet p_DataSet, int p_TableIndex, string rowNumber)
        {
            if (p_DataSet == null || p_DataSet.Tables.Count < 0)
                return null;
            if (p_TableIndex > p_DataSet.Tables.Count - 1)
            ...
            for (int j = 0; j < p_Data.Rows.Count; j++)
            {
                ...
                foreach (PropertyInfo pi in propertys)
                {
                    if (pi.Name.Equals(rowNumber))
                        pi.SetValue(_t, j, null);
                    for (int i = 0; i < p_Data.Columns.Count; i++)
                    {
                        // 属性与字段名称一致的进行赋值       
                        if (pi.Name.Equals(p_Data.Columns[i].ColumnName))
