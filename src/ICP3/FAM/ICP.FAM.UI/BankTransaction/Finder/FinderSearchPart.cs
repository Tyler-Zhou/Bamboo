...
namespace ICP.FAM.UI.BankTransaction.Finder
{
    ...
    public partial class FinderSearchPart : BaseSearchPart
    {
        ...
        /// 借贷标志
        /// </summary>
        string _DebitCreditFlag = string.Empty;
        /// <summary>
        /// 操作口岸ID
        /// </summary>
        ...
        {
            List<BankTransactionInfo> list = null;
            BankTransactionSearchParameter requestParameter = new BankTransactionSearchParameter()
            {
                ...
                FlowWaterNO = txtWaterNO.Text,
                RelativeAccountName = txtRelativeAccountNo.Text,
                MinimumAmount = 0.00m,
                MaximumAmount = 0.00m,
            ...
            foreach (var item in values)
            {
                switch (item.Key.ToUpper())
                {
                    case "INPUTVALUE":
                        ...
                        break;
                    case "COMPANYID":
                        _CompanyID = new Guid("" + item.Value);
                        break;
                    case "BANKACCOUNTID":
                        _BankAccountID = new Guid("" + item.Value);
                        break;
                    case "DEBITCREDITFLAG":
                        _DebitCreditFlag = "" + item.Value;
                        break;
                }
            }
        }
