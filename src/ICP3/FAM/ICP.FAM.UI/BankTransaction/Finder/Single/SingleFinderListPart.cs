...
namespace ICP.FAM.UI.BankTransaction.Finder
{
    ...
    public partial class SingleFinderListPart : FinderListPart
    {
        ...
        {
            var list = (List<BankTransactionInfo>)bsList.List;
            for (int i = 0; i < list.Count; i++)
            {
                if (list[i].FlowWaterNO.Trim().ToLower() == refNo.Trim().ToLower())
                {
                    bsList.Position = i;
        ...
        [CommandHandler(BankTransactionFinderConstants.COMMANDSINGLEFINDERCONFIRM)]
        public void SingleFinder_Confirm_BankTransaction(object sender, EventArgs e)
        {
            ConfirmSelected();
        }
