...
namespace ICP.FAM.UI.BankTransaction.Finder
{
    public partial class FinderListPart : BaseListPart
    {
        ...
        {
            List<BankTransactionInfo> list = DataSource as List<BankTransactionInfo>;
            if (list == null) return;
            List<BankTransactionInfo> newLists = items as List<BankTransactionInfo>;

            foreach (var item in newLists)
            {
                BankTransactionInfo tager = list.Find(delegate(BankTransactionInfo jItem) { return item.ID == item.ID; });
                if (tager == null) continue;

                Utility.CopyToValue(tager, item, typeof(BankTransactionInfo));
            }
            bsList.ResetBindings(false);
        }
