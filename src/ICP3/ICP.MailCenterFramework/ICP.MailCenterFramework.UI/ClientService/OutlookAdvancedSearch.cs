...
namespace ICP.MailCenterFramework.UI
{
    class OutlookAdvancedSearch
    {
        ...
        public string ScopeName
        {
            get
            {
                if (string.IsNullOrEmpty(_Scope))
                    _Scope = "\'" + OutlookUtility.CurrentNameSpace.DefaultStore.GetRootFolder().FolderPath + "\'";
                return _Scope;
            }
            set
            {
        ...
        public OutlookAdvancedSearch(string strIMessageID, string strSubject = "")
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(strIMessageID))
                {
                    ...
                    _Filter += "(\"" + OutlookUtility.OutlookMessageIDTag + "\" = \'" + strIMessageID + "\') "
                        + " OR  (\"" + OutlookUtility.CustomMessageIDTag + "/0000001F\"=\'" + strIMessageID + "\')";
                }
            ...
            {
                ToolUtility.WriteLog("OutlookAdvancedSearch", ex);
            }
        }
        #endregion
        ...
        public void RunAdvancedSearch()
        {
            try
            {
                if (!string.IsNullOrEmpty(_Filter))
                {
                    ...
                    OutlookUtility.CurrentApplication.AdvancedSearchComplete -= OLApplication_AdvancedSearchComplete;
                }
            }
            catch (System.Exception ex)
            {
        ...
        {
            OutlookUtility.CurrentApplication.AdvancedSearchComplete += OLApplication_AdvancedSearchComplete;
            advancedSearch = OutlookUtility.CurrentApplication.AdvancedSearch(ScopeName, _Filter, true, "");
            //高级查询未结束且未到达超时时间
