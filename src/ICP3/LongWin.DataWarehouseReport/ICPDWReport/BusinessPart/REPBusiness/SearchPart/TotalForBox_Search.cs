...
namespace LongWin.DataWarehouseReport.WinUI
{
    ...
    ///  箱量利润统计表 
    /// </summary>
    public partial class TotalForBox_Search : UserControl
    {
        #region 服务注入
        ...
        public TotalForBox_Search()
        {
            InitializeComponent();
            this.Disposed += new EventHandler(TotalForBox_Search_Disposed);
        }

        void TotalForBox_Search_Disposed(object sender, EventArgs e)
        {
            this._workItem.Items.Remove(this);
        }

        bool _isCommerce = false;
        /// <summary>
        /// 是否是商务报表
        /// </summary>
        public bool IsCommerce
        {
            get { return _isCommerce; }
            set
            {
                _isCommerce = value;
                if (!_isCommerce)
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            if (!this.DesignMode)
            {
                ...
                this.labShipAgent.Text = Utility.GetValueString("承运人", "承运人");
                ...
                //this.cmbSalesType.Items.AddRange(Utility.GetGoodsSalesTypes());

                Utility.SetGoodsSalesTypes(cmbSalesType);

                this.cmbGroupBy.Items = new string[] {  Utility.GetValueString("客户", "客户"),
                ...
                //this.cmbSalesType.SelectedIndex = 6;  
        ...
        public string CompanyCAddress
        {
            get
            {
                if (_configureDate == null)
                {
                    System.Guid ID = _initializeService.GetUserInfo().GetNormalCompany().Id;
                    if (ID == null || ID == Guid.Empty)
                    {
                        ID = new Guid(this.ucSelectCompany.Value.ToString());
                    }
                    _configureDate = _configureService.GetSystemConfig(ID);
                }
                if (_configureDate == null)
                    return string.Empty;
                else
