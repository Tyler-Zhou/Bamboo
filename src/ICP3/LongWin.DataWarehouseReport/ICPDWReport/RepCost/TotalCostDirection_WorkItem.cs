...
{
    public class TotalCostDirection_WorkItem:WorkItem
    {
        ReportMainSpace reportMainWorkSpace;
        TotalCostDirection_SearchPart _SearchPart;
        ...
        internal void Show(IWorkspace workspace)
        {
            if (this.reportMainWorkSpace == null)
            {
                ...
                info.Title = Utility.GetValueString("成本趋势分析图", "成本趋势分析图");
                ...
                {
                    this._SearchPart = this.Items.AddNew<TotalCostDirection_SearchPart>();
        ...
        void _SearchPart_SearchResult(object sender, EventArgs e)
        {
            ...
            this._paramList.Add(new ReportParameter("IsYear", this._SearchPart.IsYear.ToString()));
            this._paramList.Add(new ReportParameter("IsEnglish", Utility.IsEnglish ? "1" : "0"));
            //显示报表的用法


            this._ListPart.ReportName = "RPT_ALLCostForDirection";
