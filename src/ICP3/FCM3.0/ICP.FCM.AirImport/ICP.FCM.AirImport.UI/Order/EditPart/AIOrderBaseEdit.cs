...
namespace ICP.FCM.AirImport.UI
{
    ...
    public partial class OIOrderBaseEdit : BaseEditPart
    {
        ...
        void SetLazyLoaders()
        {
            ...
            {
                ICPCommUIHelperService.BindCustomerList(this.mcmbAirCompany, CustomerType.Airline);
            });
        }
        ...
        private void SetRecentlyOrderList()
        {
            ...
            {
                bsOrders.Clear();
                List<AirOrderInfo> orderList = oiService.GetAIRecentlyOrderList(_orderInfo.CompanyID, _orderInfo.CustomerID, LocalData.UserInfo.LoginID, 10);
        ...
        private void gvOrders_DoubleClick(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.Yes)
            {
                AirOrderInfo order = oiService.GetAIOrderInfo(CurrentOrderList.ID);
                if (order == null)
                {
                    return;
                }
        ...
        {
            this._orderInfo = oiService.GetAIOrderInfo(orderId);
        }
        ...
        void ShowOrder()
        {
            ...
            {
                feelist = oiService.GetAIOrderFeeList(_orderInfo.ID);
            }

            if (Utility.GuidIsNullOrEmpty(_orderInfo.CustomerID) == false)
            {
        ...
        private bool Save(AirOrderInfo currentData, bool isSavingAs)
        {
            ...
            try
            {
                ...
                Dictionary<Guid, SaveResponse> saved = this.oiService.SaveAIOrderWithTrans(originalOrder,
                    originalFees);
        ...
        private void AfterSave()
        {
            ...
            if (this._orderInfo.State == AIOrderState.Rejected)
            {
                ...
                if (dr == DialogResult.Yes)
                {
                    try
                    {
                        SingleResultData result = this.oiService.ChangeAIOrderState(this._orderInfo.ID, AIOrderState.NewOrder, "Changed state by user with prompt confirmation.", LocalData.UserInfo.LoginID, this._orderInfo.UpdateDate);
