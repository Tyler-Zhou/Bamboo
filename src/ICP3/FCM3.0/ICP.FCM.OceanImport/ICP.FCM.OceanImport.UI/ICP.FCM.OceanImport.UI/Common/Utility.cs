...
namespace ICP.FCM.OceanImport.UI
{
    public class Utility
    {
        ...
        {
           return  PartLoader.ShowDialog(control,title);
        }
        ...
        {
            Utility.ShowEditPart<T>(Workitem, source, null, title, editPartSaved, string.Empty);
        }
        /// <summary>
        /// 显示一个编辑页面到MainWorkspace
        /// </summary>
        ...
        /// <param name="title">Title</param>
        /// <param name="editPartSaved">编辑页面保存后执行的委托,可以传Null</param>
        /// <param name="partCode">为防止编辑的页面重复打开,可传入一个唯一的Code,在下一次打开此类型页面时，会先到Workitem中找到此Part</param>
        public static void ShowEditPart<T>(WorkItem Workitem, object source, string title, EditPartSaved editPartSaved, string partCode) where T : BaseEditPart
        {
            Utility.ShowEditPart<T>(Workitem, source, null, title, editPartSaved, partCode);
        }
        ...
        /// <param name="editPartSaved">编辑页面保存后执行的委托,可以传Null</param>
        /// <param name="partCode">为防止编辑的页面重复打开,可传入一个唯一的Code,在下一次打开此类型页面时，会先到Workitem中找到此Part</param>
        public static void ShowEditPart<T>(WorkItem Workitem, object source, IDictionary<string, object> values, string title, EditPartSaved editPartSaved, string partCode) where T : BaseEditPart
        {
            T editPart = null;

            if (string.IsNullOrEmpty(partCode) == false) editPart = Workitem.Items.Get<T>(partCode);

            IWorkspace mainWorkspace = Workitem.Workspaces[ICP.Framework.CommonLibrary.Client.ClientConstants.MainWorkspace];

            if (editPart == null)
            {
                if (string.IsNullOrEmpty(partCode) == false) editPart = Workitem.Items.AddNew<T>(partCode);
                else editPart = Workitem.Items.AddNew<T>();
            ...
            {
                mainWorkspace.Activate(editPart);
            }
        }
        ...
        /// <param name="title">Title</param>
        /// <param name="editPartSaved">编辑页面保存后执行的委托,可以传Null</param>
        public static void ShowEditPartInDialog<T>(WorkItem Workitem, object source, string title, EditPartSaved editPartSaved) where T : BaseEditPart
        {
            Utility.ShowEditPartInDialog<T>(Workitem, source, null, title, editPartSaved);
        }
        ...
        /// <param name="title">Title</param>
        /// <param name="editPartSaved">编辑页面保存后执行的委托,可以传Null</param>
        public static void ShowEditPartInDialog<T>(WorkItem Workitem, object source, IDictionary<string, object> values, string title, EditPartSaved editPartSaved) where T : BaseEditPart
        {
            ...
            Utility.ShowDialog(editPart, title);
