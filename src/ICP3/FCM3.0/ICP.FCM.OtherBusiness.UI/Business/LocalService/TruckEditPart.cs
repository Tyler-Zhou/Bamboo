...
namespace ICP.FCM.OtherBusiness.UI.Business
{
    ...
    public partial class TruckEditPart  : BaseEditPart
    {
        ...
        private void InitControls()
        {
            ...
            List<ICP.Common.ServiceInterface.DataObjects.ContainerList> ctnList = tfService.GetContainerList(string.Empty, true, 0);
        ...
        private bool Save()
        {
            ...
            try
            {
                foreach (TruckInfo truckItem in list)
                {
                    ...
                    result = obService.SaveOBTruckServiceInfo(saveRequest, LocalData.IsEnglish);
        ...
        private void Delete()
        {
            ...
            if (PartLoader.EnquireIsDeleteCurrentData())
            {
                ...
                try
                {
                    obService.RemoveOBTruckServiceInfo(currentData.ID, LocalData.UserInfo.LoginID, currentData.UpdateDate, LocalData.IsEnglish);
                    bsOceanTruckList.RemoveCurrent();
        ...
        private void Print()
        {
            ...
            {              
                //国内派车单
                OBExportPrintHelper.PrintPickupCN(_CurrentData.ID);
            }
            else
            {
        ...
        public override void Init(IDictionary<string, object> values)
        {
            ...
            foreach (var item in values)
            {
                if (item.Key.ToUpper() == "Booking".ToUpper())
                {
                    ...
                    _OceanBooking = obService.GetOtherBusinessInfo(bookingList.ID, LocalData.IsEnglish);
                }
