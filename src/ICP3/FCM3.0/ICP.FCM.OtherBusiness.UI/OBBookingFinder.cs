...
namespace ICP.FCM.OtherBusiness.UI.Booking.Finder
{
    ...
    public class OBBookingFinder : IDataFinder,IDisposable
    {
        ...
        [ServiceDependency]
        public IOtherBusiness oeService { get; set; }
        ...
        {
            DeckWorkspace workspce = this.Workitem.Workspaces.Get<DeckWorkspace>(OBBookingFinderWorkspace);
            if (workspce == null || workspce.IsDisposed)
            {
                if (this.Workitem.Workspaces.Contains(OBBookingFinderWorkspace))
                {
                    this.Workitem.Workspaces.Remove(workspce);
                }

                workspce = this.Workitem.Workspaces.AddNew<DeckWorkspace>(OBBookingFinderWorkspace);
        ...
        public void PickOne(string searchValue, string property, SearchConditionCollection conditions, string[] returnFields, FinderTriggerType triggerType, string workspaceName)
        {
            ...
            List<OtherBusinessList> list = oeService.GetOtherBusinessList(companyIDs.ToArray(),
                                                                       null,
                                                                       bookingNo,
                                                                       string.Empty,
                                                                       string.Empty,
