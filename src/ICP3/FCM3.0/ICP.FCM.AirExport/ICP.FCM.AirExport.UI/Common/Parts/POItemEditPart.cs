...
namespace ICP.FCM.AirExport.UI
{
    public partial class POItemEditPart : DevExpress.XtraEditors.XtraUserControl, IChildPart
    {
        ...
        private void DeleteData()
        {
            ...
            {
                //if (IsOrderPO)
                    OeService.RemoveAirOrderPOInfo(CurrentRow.ID, LocalData.UserInfo.LoginID, CurrentRow.UpdateDate);
                //else
        ...
        public void InitData(Guid orderId)
        {
            ...
            {                
                ...
                List<AirBookingPOList> _oceanBookingPOInfo = this.DataSource as List<AirBookingPOList>;
                if (_oceanBookingPOInfo == null)
                {
                    _oceanBookingPOInfo = this.OeService.GetAirOrderPOList(this.OrderId);
            ...
            {
                this.SetSource(new List<AirBookingPOList>());
            }
        }

        void item_PropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            List<AirBookingPOList> pos = this.DataSource as List<AirBookingPOList>;
            if (pos == null)
            {
                return ;
            }
