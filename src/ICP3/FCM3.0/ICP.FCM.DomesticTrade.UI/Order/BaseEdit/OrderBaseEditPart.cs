...
namespace ICP.FCM.DomesticTrade.UI.Order
{
    ...
    public partial class OrderBaseEditPart : BaseEditPart
    {
        ...
        void ReadyForNew()
        {
            ...
            this._orderInfo.DTOperationType = FCMOperationType.FCL;
        ...
        void PrepareForCopyExistOrder()
        {
            ...
            this._orderInfo.State = DTOrderState.NewOrder;
        ...
        void SetState()
        {
            this.txtState.Text = EnumHelper.GetDescription<DTOrderState>(this._orderInfo.State, LocalData.IsEnglish);
        }
        ...
        private void InitControls()
        {
            ...
            //业务类型
            this.cmbType.ShowSelectedValue(this._orderInfo.DTOperationType,
                EnumHelper.GetDescription<FCMOperationType>(this._orderInfo.DTOperationType, LocalData.IsEnglish));
        ...
        {
            if (this._orderInfo.SODate.HasValue)
            {
        ...
        void cmbType_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            this.chkIsTruck.Enabled = this._orderInfo.DTOperationType == FCMOperationType.FCL;
        ...
        private void SetAgentByCompany()
        {
            if (string.IsNullOrEmpty(this._orderInfo.AgentName))
            {
                ...
                {
                    if (!oeService.IsCustomerAndCompanySameCountry(this._orderInfo.CustomerID, this._orderInfo.CompanyID, LocalData.IsEnglish))
                    {
        ...
        void mcmOverseasFiler_Click(object sender, EventArgs e)
        {
            ...
            ICPCommUIHelperService.SetComboxUsersByRole(mcmOverseasFiler, depID, "海外拓展", true);
        ...
        private void SetSalesTypeByCustomerAndCompany()
        {
            ...
            {
                DataDictionaryInfo salesType = oeService.GetSalesType(_orderInfo.CustomerID, _orderInfo.CompanyID, LocalData.IsEnglish);
                if (salesType != null)
                {
        ...
        private void SetRecentlyOrderListByCustomerAndCompany()
        {
            ...
            {
                bsOrders.Clear();
                List<DTOrderList> orderList = oeService.GetRecentlyDTOrderList(_orderInfo.CompanyID, _orderInfo.CustomerID, 10, LocalData.IsEnglish);
        ...
        private void gvOrders_DoubleClick(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.Yes)
            {
                DTOrderInfo order = oeService.GetDTOrderInfo(CurrentOrderList.ID, LocalData.IsEnglish);
                if (order == null)
                {
                    return;
                }
        ...
        {
            this._orderInfo = oeService.GetDTOrderInfo(orderId, LocalData.IsEnglish);
        }
        ...
        void ShowOrder()
        {
            ...
            {
                feelist = oeService.GetDTOrderFeeList(_orderInfo.ID, LocalData.IsEnglish);
            }
        ...
        void BindingData(object data)
        {
            ...
            if (listInfo == null)
            {
                this._orderInfo = new DTOrderInfo();
                this.ReadyForNew();
            }
            else
            {
        ...
        private bool Save(DTOrderInfo currentData, bool isSavingAs)
        {
            ...
            try
            {
                ...
                Dictionary<Guid, SaveResponse> saved = this.oeService.SaveDTOrderWithTrans(originalOrder,
                    originalFees, LocalData.IsEnglish);
        ...
        {
            if (_orderInfo == null || Utility.GuidIsNullOrEmpty(_orderInfo.BookingerID))
            {
                return;
            }
            ...
            try
            {
                ...
                //if (LocalData.IsEnglish) fileName = Path.Combine(fileName, "DT_OrderInfo_EN.frx");
                //else fileName = Path.Combine(fileName, "DT_OrderInfo_CN.frx");
                ...
                //数据源
                DTOrderReportData data = DTReportSrvice.GetDTOrderReportData(_orderInfo.ID);
                if (data == null) return;
        ...
        ICP.Message.ServiceInterface.Message CreateMessageInfo(string title, string content)
        {
            ...
            userPropertiesObject.OperationType = ICP.Framework.CommonLibrary.Common.OperationType.Internal;
        ...
        private void AfterSave()
        {
            ...
            if (this._orderInfo.State == DTOrderState.Rejected)
            {
                ...
                if (dr == DialogResult.Yes)
                {
                    try
                    {
                        SingleResult result = this.oeService.ChangeDTOrderStateWithTargetState(this._orderInfo.ID, DTOrderState.NewOrder, "Changed state by user with prompt confirmation.", LocalData.UserInfo.LoginID, this._orderInfo.UpdateDate, LocalData.IsEnglish);

                        this._orderInfo.State = (DTOrderState)result.GetValue<byte>("State");
            ...
            if (Saved != null)
            {
                ...
                this._orderInfo.BookingTypeDescription = EnumHelper.GetDescription<FCMOperationType>(this._orderInfo.DTOperationType, LocalData.IsEnglish);
