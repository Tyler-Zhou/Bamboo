...
namespace ICP.FCM.DomesticTrade.UI
{
    public partial class POItemSelectPart : DevExpress.XtraEditors.XtraUserControl, IChildPart
    {
        ...
        [ServiceDependency]
        public IDomesticTradeService OeService { get { return Workitem.Services.Get<IDomesticTradeService>(); } }
        ...
        public void SetAllPO(Guid orderId, WorkItem workItem, Guid containerId)
        {
            if (this._orderId == Guid.Empty)
            {
                ...
                CompositePoAndItems pos = OeService.GetDTContainerPOList(orderId,LocalData.IsEnglish);

                _pos = pos;

                foreach (DTBookingPOList po in this._pos.PoList)
                {
        ...
        public List<POSaveRequest> CollectPosData()
        {
            ...
            if (this._pos != null && this._pos.PoList.Count > 0)
            {
                if (this.IsChanged)
                {
                    ...
                    List<DTBookingPOList> _listHasChanged = this._pos.PoList.FindAll(o => o.IsDirty);
        ...
        void item_PropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            ...
            //{
                DTBookingPOList po = this._pos.PoList.First(o => o.ID == item.POID);
                if (po != null)
                {
                    po.IsDirty = true;
                }
        ...
        private void DeleteData()
        {
            ...
            if (this.gridControl1.FocusedView == this.gridViewPO)
            {
                ...
                {
                    OeService.RemoveDTContainerPOInfo(CurrentRow.ID, LocalData.UserInfo.LoginID, CurrentRow.UpdateDate,LocalData.IsEnglish);
                }
