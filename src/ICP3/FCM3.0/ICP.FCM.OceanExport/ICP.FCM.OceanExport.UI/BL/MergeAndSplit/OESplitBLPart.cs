...
namespace ICP.FCM.OceanExport.UI.BL
{
    public partial class OESplitBLPart : BaseEditPart
    {
        ...
        {
            dfService.RegisterGridColumnFinder(colShipper
            ...
            dfService.RegisterGridColumnFinder(colConsignee
                                              , ICP.Common.ServiceInterface.CommonFinderConstants.CustoemrFinder
                                              , "ConsigneeID"
                                              , "ConsigneeName"
                                              , "ID"
            ...
            dfService.RegisterGridColumnFinder(colNotifyParty
                                              , ICP.Common.ServiceInterface.CommonFinderConstants.CustoemrFinder
                                              , "NotifyPartyID"
                                              , "NotifyPartyName"
                                              , "ID"
        ...
        private void bsList_PositionChanged(object sender, EventArgs e)
        {
            ...
            if (CurrentRow.Containers == null)
            {
                ...
                {
                    ClientContainerList c = Utility.Clone<ClientContainerList>(item.Value);
        ...
        {
            //复制首条
            ClientSplitBLInfo list = Utility.Clone<ClientSplitBLInfo>(_CurrentSource[0]);
        ...
        public override bool SaveData()
        {
            ...
            try
            {
                ...
                ManyResult result = oeService.SplitOceanBL(doc.ToString(),LocalData.UserInfo.LoginID);
        ...
        {
            #region 原始数据
            _OrgData = data as OceanBLList;
            bsOrg.DataSource = _OrgData;

            //箱
            List<OceanBLContainerList> ctns = null;
            if (_BLType == FCMBLType.MBL)
                ctns = oeService.GetOceanMBLContainerList(_OrgData.ID);
            else
                ctns = oeService.GetOceanHBLContainerList(_OrgData.ID);
            ...
            Utility.CopyToValue(_OrgData, info, typeof(ClientSplitBLInfo));
            if (_BLType == FCMBLType.HBL)
            {
                _MBLs = oeService.GetOceanBookingListByIds(new Guid[] { _OrgData.OceanBookingID })[0].OceanMBLs;
            ...
            foreach (var item in ctns)
            {
                ...
                Utility.CopyToValue(item, c, typeof(ClientContainerList));
                info.Containers.Add(c);
                //复制一条到原始数据
                _OrgCtn.Add(c.ID ,Utility.Clone<ClientContainerList>(c));
            ...
            bsContainer.DataSource = info.Containers;
