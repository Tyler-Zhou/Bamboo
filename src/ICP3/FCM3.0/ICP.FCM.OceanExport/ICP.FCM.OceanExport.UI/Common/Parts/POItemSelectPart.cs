...
namespace ICP.FCM.OceanExport.UI
{
    public partial class POItemSelectPart : DevExpress.XtraEditors.XtraUserControl, IChildPart
    {
        ...
        [ServiceDependency]
        public IOceanExportService OeService { get { return Workitem.Services.Get<IOceanExportService>(); } }
        ...
        public void SetAllPO(Guid orderId, WorkItem workItem, Guid containerId)
        {
            if (this._orderId == Guid.Empty)
            {
                ...
                CompositePoAndItems pos = OeService.GetOceanContainerPOList(orderId);

                _pos = pos;

                foreach (OceanBookingPOList po in this._pos.PoList)
                {
        ...
        public List<POSaveRequest> CollectPosData()
        {
            ...
            if (this._pos != null && this._pos.PoList.Count > 0)
            {
                if (this.IsChanged)
                {
                    ...
                    List<OceanBookingPOList> _listHasChanged = this._pos.PoList.FindAll(o => o.IsDirty);

                    #region Save

                    for (int i = 0; i < _listHasChanged.Count; i++)
                    {
                        OceanBookingPOList poItem = _listHasChanged[i];
        ...
        public void RefreshUI(List<POSaveRequest> list)
        {
            foreach (POSaveRequest saveRequest in list)
            {
                ...
                {
                    OceanBookingPOList po = saveRequest.UnBoxInvolvedObject<OceanBookingPOList>()[0];
        ...
        void item_PropertyChanged(object sender, PropertyChangedEventArgs e)
        {
            ...
            //{
                OceanBookingPOList po = this._pos.PoList.First(o => o.ID == item.POID);
                if (po != null)
                {
                    po.IsDirty = true;
                }
        ...
        private void DeleteData()
        {
            ...
            if (this.gridControl1.FocusedView == this.gridViewPO)
            {
                ...
                {
                    OeService.RemoveOceanContainerPOInfo(CurrentRow.ID, LocalData.UserInfo.LoginID, CurrentRow.UpdateDate);
                }
        ...
        void ShowPoList()
        {
            ...
            List<OceanBookingPOList> results = new List<OceanBookingPOList>();
            ...
            foreach (OceanBookingPOList po in results)
            {
