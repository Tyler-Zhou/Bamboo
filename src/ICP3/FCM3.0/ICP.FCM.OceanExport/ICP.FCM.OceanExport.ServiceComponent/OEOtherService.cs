...
namespace ICP.FCM.OceanExport.ServiceComponent
{
    partial class OceanExportService
    {
        ...
        /// <returns></returns>
        public bool IsCarrierInCompanyEdiBooking(Guid carrierId, Guid companyId)
        {
        ...
        public bool IsPortCountryExistCompanyConfig(Guid portId, Guid? companyId)
        {
            ...
            try
            {
                ...
                if (companyId == null)
                {
                    isExist=Convert.ToBoolean(ds.Tables[0].Rows[0][0].ToString())==true;
                }
                else
                {
                    isExist = Convert.ToBoolean(ds.Tables[0].Rows[0][0].ToString()) == false;
                }
                return isExist;
            }
            catch (SqlException sqlException)
